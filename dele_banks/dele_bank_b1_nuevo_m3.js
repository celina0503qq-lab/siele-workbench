window.DELE_BANK_B1_NUEVO_M3 = {
  "version": "2.0",
  "level": "B1",
  "source": "Nuevo DELE B1 Modelo 3 — Ramón Díez Galán. Reconstrucción íntegra desde el PDF: lectura 30 + auditiva 30 (con transcripciones) + escritura 2 + expresión oral 4 (incluida la Tarea 4 de diálogo).",
  "language": "es-ES",
  "syllabus": {
    "level": "B1",
    "duration_minutes": {
      "reading": 70,
      "listening": 40,
      "writing": 60,
      "speaking_prep": 15,
      "speaking_test": 15
    }
  },
  "items": [
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿A qué texto corresponde Yolanda (persona 1)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 2,
      "q_range": [
        1,
        6
      ],
      "answer": "D",
      "prompt": "¿A qué texto corresponde Álex (persona 2)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: D",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 3,
      "q_range": [
        1,
        6
      ],
      "answer": "F",
      "prompt": "¿A qué texto corresponde Inés (persona 3)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: F",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 4,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿A qué texto corresponde Héctor (persona 4)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 5,
      "q_range": [
        1,
        6
      ],
      "answer": "I",
      "prompt": "¿A qué texto corresponde Pablo (persona 5)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: I",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 1,
      "q": 6,
      "q_range": [
        1,
        6
      ],
      "answer": "G",
      "prompt": "¿A qué texto corresponde Natalia (persona 6)?",
      "options": [
        {
          "key": "A",
          "text": "Mensaje:. Escúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa."
        },
        {
          "key": "B",
          "text": "Mensaje:. La verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo."
        },
        {
          "key": "C",
          "text": "Mensaje:. Lo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?"
        },
        {
          "key": "D",
          "text": "Mensaje:. Perdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?"
        },
        {
          "key": "E",
          "text": "Mensaje:. Vi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa."
        },
        {
          "key": "F",
          "text": "Mensaje:. ¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos."
        },
        {
          "key": "G",
          "text": "Mensaje:. Perdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas."
        },
        {
          "key": "H",
          "text": "Mensaje:. Permíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar."
        },
        {
          "key": "I",
          "text": "Mensaje:. Ayer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: G",
      "cuerpo": "PERSONAS:\n1. Yolanda: Mi jefe es demasiado sincero, ayer me dijo que no trabajo bien y que están buscando a otra persona para mi puesto.\n2. Álex: Mi amigo está en casa, dice que le duele mucho la barriga y que necesita ayuda. Voy a ver qué puedo hacer por él.\n3. Inés: Voy a empezar a tomarme la píldora, lo he hablado con mi marido y he pensado que es la mejor solución, ya me he quedado embarazada tres veces y no quiero más.\n4. Héctor: Mi sobrino está en el hospital, ayer se hizo una quemadura muy fea. Espero que no sea nada y que se ponga bien pronto. Estamos muy preocupados.\n5. Pablo: Llevo barba desde hace cinco años, pero ahora mi novia quiere que me la quite. No sé si debería hacerle caso, a mí me gusta mi barba.\n6. Natalia: Mi amiga siempre llega con retraso cuando quedamos, esta vez me ha puesto una excusa con el tráfico, ¿qué será lo siguiente? 78\n\nTEXTOS:\nA. Mensaje:\nEscúchame, quiero decirte que no estamos muy contentos con tu rendimiento y hemos decidido despedirte. Estamos teniendo entrevistas con varios candidatos, pronto encontraremos a alguien para sustituirte. Solo lo digo para avisarte y que no te pille por sorpresa.\n\nB. Mensaje:\nLa verdad es que no sé qué pasó, le he dicho mil veces durante su vida que no puede jugar con fuego. Yo pensaba que lo había entendido, pero parece que no. Fuimos a urgencias y allí le atendieron, ahora mismo está ingresado. Por favor, llama a tu hermana y díselo.\n\nC. Mensaje:\nLo he consultado con mi padre y creo que deberíamos contratar un seguro médico. Yo me quedaría más tranquilo, sobre todo cada vez que salgamos del país. ¿Has mirado los precios de las aseguradoras que te dije?\n\nD. Mensaje:\nPerdón por llamarte tan temprano, estoy teniendo problemas de estómago, me he puesto muy malo, creo que me va a ser imposible salir. Empecé anoche con diarrea y desde entonces no he podido parar de ir al baño. ¿Qué opinas? ¿Qué debo hacer?\n\nE. Mensaje:\nVi a tu primo y me sorprendí, ha cambiado un montón, ahora tiene muy buena figura. Creo que ha estado yendo al gimnasio porque tiene muchos músculos y nada de barriga. Además, estaba súper moreno, quizás se haya pasado todo el verano en la playa.\n\nF. Mensaje:\n¿Has pensado en lo que te comenté el otro día? Tenemos que encontrar otra solución, sabes que no puedo utilizar preservativos porque tengo una reacción alérgica. Seguro que hay alguna forma de continuar manteniendo relaciones sexuales sin tener más hijos.\n\nG. Mensaje:\nPerdona, lo siento mucho. No te vas a creer lo que pasa, estoy en un atasco horrible. Han puesto un semáforo nuevo cerca del puente viejo y está todo bloqueado. Además, creo que ha habido un accidente y esto ha empeorado las cosas.\n\nH. Mensaje:\nPermíteme que te hable con sinceridad. Tu problema está muy claro, es la alimentación. ¿Cómo pretendes que cambie algo si sigues teniendo esa dieta que abusa de las proteínas? Deberías empezar a informarte sobre los productos ligeros. Conozco a una persona que te puede ayudar.\n\nI. Mensaje:\nAyer estuve en la tienda y te compré maquinillas y espuma de afeitar, lo he dejado todo en el primer cajón del mueble del baño. Si me quieres ya sabes lo que tienes que hacer. Nos vemos esta noche, un beso. 79",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes que estas personas han recibido. Relacione a las personas (1-6) con los textos que informan sobre los mensajes (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 7,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Sharon Stone piensa que…",
      "options": [
        {
          "key": "A",
          "text": "las personas que dicen que en Hollywood no importa la edad dicen la verdad."
        },
        {
          "key": "B",
          "text": "el aspecto físico y la edad son muy importantes para trabajar como actriz."
        },
        {
          "key": "C",
          "text": "después de cumplir 60 años tienes mejores contratos en Hollywood."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 8,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "El texto dice que…",
      "options": [
        {
          "key": "A",
          "text": "Sharon Stone está feliz con su cuerpo."
        },
        {
          "key": "B",
          "text": "la hija de Sharon Stone tiene 20 años."
        },
        {
          "key": "C",
          "text": "durante la cuarentena, Sharon Stone se mudó de América."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 9,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "La actriz Sharon Stone…",
      "options": [
        {
          "key": "A",
          "text": "debutó como actriz con la película “Instinto básico”."
        },
        {
          "key": "B",
          "text": "olvidó ponerse ropa interior al hacer una película."
        },
        {
          "key": "C",
          "text": "actuó en una película de Woody Allen en 1980."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 10,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "Según el texto, Sharon Stone…",
      "options": [
        {
          "key": "A",
          "text": "siempre da su opinión, aunque pueda resultar polémica."
        },
        {
          "key": "B",
          "text": "ha tenido varias relaciones amorosas con productores de Hollywood."
        },
        {
          "key": "C",
          "text": "siempre estaba muy tranquila durante sus trabajos como actriz."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 11,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "El texto dice que…",
      "options": [
        {
          "key": "A",
          "text": "el movimiento #MeToo fue iniciado por Sharon Stone."
        },
        {
          "key": "B",
          "text": "el movimiento #MeToo mostró malos comportamientos de diferentes profesionales del cine."
        },
        {
          "key": "C",
          "text": "Sharon Stone jamás tuvo una mala experiencia al actuar."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 2,
      "q": 12,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "Sharon Stone…",
      "options": [
        {
          "key": "A",
          "text": "no ha continuado con su carrera profesional desde el siglo pasado."
        },
        {
          "key": "B",
          "text": "ha ganado en varias ocasiones el premio Óscar."
        },
        {
          "key": "C",
          "text": "sigue realizando trabajos como actriz."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Sharon Stone: “la apariencia física sí importa en Hollywood”\nEn tiempos de igualdad, de inclusión y de valorización de las cualidades artísticas en\nHollywood, la actriz estadounidense Sharon Stone manifestó que en la industria del cine aún\nse valora mucho a la apariencia física, la belleza y la juventud, por encima del talento. \"Quien\ndice lo contrario es un mentiroso e hipócrita\", expresó.\n\"No te das cuenta de lo mucho que la apariencia importa hasta que empiezas a desaparecer,\ncuando ya no trabajas, cuando ya no te llaman para hacer películas\", manifestó la actriz.\nRecordó que la industria del cine vende belleza\ny juventud, cosas que muchos quieren tener y\nque se satisfacen al verlo en la pantalla. Las\nhistorias son creadas para gente bonita y joven,\nintentar actuar después de los 60 años es casi\nimposible, aclaró.\nStone comentó que ya no le importa lo que otras personas digan de su cuerpo o su cara.\nTiene 62 años y se siente muy bien con su físico, que a su edad no pretende lucir como una\nchica de 20 años. Durante el tiempo de cuarentena realizaba ejercicios físicos en su casa, en\nCalifornia, EE UU, para ayudar a conservar su salud, que para ella es más importante que la\napariencia.\nAunque la carrera de Sharon Stone empezó en 1980 con “Los diarios del ron”, de Woody\nAllen, fue en 1992, con la polémica película “Instinto básico” cuando se hizo realmente\nfamosa, gracias a las escenas sensuales que protagonizaba. Se recuerda el momento cuando\nStone, sin llevar ropa interior, cruza las piernas estando sentada. Dicha escena es\nconsiderada una de las más influyentes de la historia del cine, por su carga erótica, sin llegar\na mostrar las partes íntimas de la actriz. De esa imagen ella ha querido deshacerse durante\nmucho tiempo, llegando a decir que tiene mejores trabajos en el cine, sin embargo, el público\ny los críticos la siguen recordando por esto.\nSharon Stone es una artista polémica. Algunos productores no querían trabajar con ella,\ndecían que causaba problemas. Y es que casi siempre dice lo que piensa y, cuando siente\ninjusticias, lo expresa públicamente.\nFue una de las estrellas de Hollywood que colaboró con el movimiento #MeToo, que exhibió los\nabusos laborales y sexuales a las actrices por parte de productores, realizadores y hasta de\notros actores. Contó que una vez un director le hizo sentarse en sus piernas para explicarle algo\nsobre la película. En otra ocasión, un actor le dijo durante el trabajo que quería ver sus pechos,\ny así como esas situaciones vivió muchas.\nEn 1996 fue nominada al Óscar y recibió el premio Globo de Oro por la película Casino, de\nMartin Scorsese. Su trabajo más reciente en el cine fue en 2018, “What about love”, y este año\nha presentado en la televisión el documental “El nuevo papa”.\nAdaptado de: www.eldeber.com",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre la apariencia física. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 13,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "¿Qué persona sale a caminar tranquilamente para hacer algo de ejercicio?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 14,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "¿Qué persona comenta que sufrió una lesión por la que tuvo que estar ingresada?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 15,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "¿Quién dice que se cansaba mucho cuando hacía ejercicio?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 16,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "¿Quién dice que de adolescente quería tener la figura ideal?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 17,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "¿Quién dice que sus amigos entrenan en un gimnasio?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 3,
      "q": 18,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "¿Qué persona dice que se dedica al sector de la ley y la justica?",
      "options": [
        {
          "key": "A",
          "text": "EMI"
        },
        {
          "key": "B",
          "text": "MAX"
        },
        {
          "key": "C",
          "text": "LUZ"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "TEXTOS:\nA. EMI\nCuando tenía quince años estaba loca por mi imagen, cada día hacía ejercicios para perder peso y tener la barriga plana como las chicas de las portadas de las revistas. Al cumplir veinte años empecé a trabajar en una oficina y, no sé si fue porque perdí el interés o por falta de tiempo, pero dejé de entrenar. Ahora he descubierto que el aspecto físico no es algo con lo que debamos obsesionarnos, soy feliz dando mis paseos diarios y bailando en mis clases de salsa que tengo una vez a la semana. Lo que hacía antes me tenía atrapada en un ciclo sin fin, nunca estaba contenta y siempre quería entrenar más y más.\n\nB. MAX\nUn físico perfecto solo se consigue con mucho esfuerzo y sacrificio. Cuando iba a la universidad, me pagaba los estudios con lo que ganaba haciendo de entrenador personal en un gimnasio de las afueras. Para mí, empezó como una obligación, pero poco a poco se ha convertido en una afición. He conocido a casi todas mis amistades entre las máquinas del club de fitness. Ahora ya no me dedico a esto profesionalmente, pero continúo con mi plan intensivo de ejercicios semanales. Estudié derecho y ahora trabajo en un prestigioso bufete internacional. Es un poco difícil organizar mi empleo y mi entrenamiento, pero poco a poco lo estoy consiguiendo.\n\nC. LUZ\nHace ya mucho tiempo que no lo hago, pero antes practicaba atletismo tres veces a la semana. Yo lo odiaba, pero mis padres me obligaban a ir a esas horribles clases, eran muy duras, el entrenador quería preparar a futuras estrellas, atletas profesionales, y no era lo mío. Lo único que conseguía era estar agotada en mi día a día. Recuerdo que una vez me rompí la rodilla derecha, estuve en el hospital más de tres semanas, me dieron el alta justo antes de los exámenes finales. Por supuesto, los suspendí todos, ese fue el momento en el que mis padres apoyaron mi decisión de dejar los entrenamientos. 84",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus vidas. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 19,
      "q_range": [
        19,
        24
      ],
      "answer": "D",
      "prompt": "Pregunta 19",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: D",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 20,
      "q_range": [
        19,
        24
      ],
      "answer": "B",
      "prompt": "Pregunta 20",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 21,
      "q_range": [
        19,
        24
      ],
      "answer": "F",
      "prompt": "Pregunta 21",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: F",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 22,
      "q_range": [
        19,
        24
      ],
      "answer": "H",
      "prompt": "Pregunta 22",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: H",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 23,
      "q_range": [
        19,
        24
      ],
      "answer": "A",
      "prompt": "Pregunta 23",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 4,
      "q": 24,
      "q_range": [
        19,
        24
      ],
      "answer": "G",
      "prompt": "Pregunta 24",
      "options": [
        {
          "key": "A",
          "text": "Cuestiónalo todo, mira directamente los ingredientes y la composición nutricional para saber qué contiene realmente lo que compras"
        },
        {
          "key": "B",
          "text": "Eres el ejemplo de tus hijos, sobrinos o nietos y es importante que te vean comer cosas saludables y disfrutarlas porque entonces les será más fácil entender que eso es algo bueno."
        },
        {
          "key": "C",
          "text": "Cuando se lo comentaron dejó la dieta inmediatamente para volver a su ritmo de vida normal"
        },
        {
          "key": "D",
          "text": "Mi experiencia tras acompañar a varias personas a lo largo de esta transición es que han conseguido un mejor peso"
        },
        {
          "key": "E",
          "text": "Debido a estas enfermedades, los nutricionistas están cada vez más presentes en los medios de comunicación"
        },
        {
          "key": "F",
          "text": "En todos los tipos de alimentación, hagas la dieta que hagas"
        },
        {
          "key": "G",
          "text": "Es importante que tu dieta esté bien planificada y controlada por este profesional de la salud."
        },
        {
          "key": "H",
          "text": "Si estás mejorando tus hábitos no te compares con nadie 86"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: G",
      "cuerpo": "¿QUÉ ES COMER SANO?\nHe hablado mucho sobre la industria alimentaria, sobre no creer en los envases ni en los\nanuncios. ¿Y ahora qué? No se trata de morirse de hambre ni mucho menos, comer sano y\nequilibrado es comer mucho, pero mucho de otras cosas.\n…………(19)…………..., se han encontrado mejor, con más energía, con menos problemas\ndigestivos, mejor descanso nocturno y pelo, piel y uñas más sanas entre otros beneficios.\nPero las mayores ventajas de una vida saludable están a largo plazo ya que nos ayudan a\nprevenir enfermedades no transmisibles que, a día de hoy, se están extendiendo a gran\nvelocidad. Estas enfermedades son la diabetes, las enfermedades cardiovasculares y los\naccidentes cerebrovasculares e incluso el cáncer.\nOtra de las grandísimas ventajas de comer sano y\nfamiliarizarte con la nutrición es que serás el\nejemplo para los que te rodean, los que verán cómo\nha cambiado tu vida y cómo te sientes y lo más\nimportante… los más pequeños. …………(20)…………...\nConsumir ensalada y pollo a la plancha pensando que es algo saludable es un error, debido a\nque el consumo de carne está limitado semanalmente. …………(21)…………..., los grandes\nprotagonistas deben ser los vegetales. Básicamente, toma el volumen de medio plato de\nvegetales en la comida y en la cena y reparte a lo largo del día, como mínimo, 3 raciones de\nfrutas al día.\n…………(22)…………... Hay personas que comen realmente mal, pero que ellos lo hagan no ha de\nservirte a ti de consuelo para interpretar que lo que tú hacías no está tan mal.\nSé muy crítico con los mitos, con lo de “me han dicho que esto va mal para esto” o “de esto hay\nque comer mucho porque tiene de lo otro” o “si te tomas esto adelgazas”. Y qué decir de la\npublicidad, los envases y la industria alimentaria en general. …………(23)…………...",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 25,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "…? ¿Has conseguido perder algo de peso? Me _____ tu marido que lo llevabas m…",
      "options": [
        {
          "key": "A",
          "text": "dijo"
        },
        {
          "key": "B",
          "text": "preguntó"
        },
        {
          "key": "C",
          "text": "dije"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 26,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "…en y que estabas muy contenta. Quiero que _____ que todas las amigas te apo…",
      "options": [
        {
          "key": "A",
          "text": "sabes"
        },
        {
          "key": "B",
          "text": "sabrías"
        },
        {
          "key": "C",
          "text": "sepas"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 27,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "…zar una dieta, pero no duró ni tres días. _____ gente que no puede controla…",
      "options": [
        {
          "key": "A",
          "text": "Se"
        },
        {
          "key": "B",
          "text": "Hay"
        },
        {
          "key": "C",
          "text": "Está"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 28,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "…stas personas. He pensado empezar a hacer _____ , pero me falta motivación p…",
      "options": [
        {
          "key": "A",
          "text": "ejercito"
        },
        {
          "key": "B",
          "text": "ejercicio"
        },
        {
          "key": "C",
          "text": "ejército"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 29,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "…os ir dos o tres días a la semana después _____ trabajo. Mi hermana me reco…",
      "options": [
        {
          "key": "A",
          "text": "del"
        },
        {
          "key": "B",
          "text": "de"
        },
        {
          "key": "C",
          "text": "el"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 1,
      "tarea": 5,
      "q": 30,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "…mba y de aeróbic, así que no creo que nos _____ Dime qué piensas, yo voy a…",
      "options": [
        {
          "key": "A",
          "text": "abramos"
        },
        {
          "key": "B",
          "text": "aburramos"
        },
        {
          "key": "C",
          "text": "encontremos"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Querida Miriam,\n¿Qué tal te va la dieta? ¿Has conseguido perder algo de peso? Me …..25….. tu marido que lo\nllevabas muy bien y que estabas muy contenta. Quiero que …..26….. que todas las amigas te\napoyamos un montón. ¿Te acuerdas de mi sobrina Eva? También intentó empezar una dieta,\npero no duró ni tres días. …..27….. gente que no puede controlar su amor por los dulces y ella es\nuna de estas personas.\nHe pensado empezar a hacer …..28….., pero me falta motivación para ir sola al gimnasio, ¿qué te\nparece si nos apuntamos juntas? Creo que te puede venir bien complementar tu dieta con un\npoco de entrenamiento físico. Podríamos ir dos o tres días a la semana después …..29…..\ntrabajo. Mi hermana me recomendó hace tiempo el gimnasio que está cerca del puente viejo,\n¿lo conoces? Tienen clases de zumba y de aeróbic, así que no creo que nos …..30…... Dime qué\npiensas, yo voy a buscar en Internet a ver si encuentro los precios del gimnasio.\nUn beso,\nÁngela.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        6
      ],
      "answer": "C",
      "prompt": "¿Qué dice esta persona?",
      "options": [
        {
          "key": "A",
          "text": "Que su hijo hace las cosas igual que su abuelo."
        },
        {
          "key": "B",
          "text": "Que su abuelo tiene mal carácter."
        },
        {
          "key": "C",
          "text": "Que su hijo es igual físicamente que un familiar."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Todo el mundo me dice que mi hijo Rafa se parece muchísimo a su abuelo\ncuando tenía su edad, he visto algunas fotos y son como dos gotas de agua. Tienen la misma\nmirada. Pero el carácter, sin duda, lo ha heredado de mí.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 2,
      "q_range": [
        1,
        6
      ],
      "answer": "C",
      "prompt": "¿Cómo estaba la persona que habla antes de escuchar el ruido?",
      "options": [
        {
          "key": "A",
          "text": "De pie."
        },
        {
          "key": "B",
          "text": "Durmiendo."
        },
        {
          "key": "C",
          "text": "Sentada."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Cuando todo pasó yo estaba en una silla, al escuchar el golpe me levanté\nrápidamente y fui al garaje para ver qué había sucedido. Allí me encontré a mi hermano en el\nsuelo, se había caído de la moto.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 3,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿Qué le pasa a esta persona?",
      "options": [
        {
          "key": "A",
          "text": "Está embarazada."
        },
        {
          "key": "B",
          "text": "Está contenta porque va a trabajar en una oficina."
        },
        {
          "key": "C",
          "text": "Está empezando un nuevo trabajo en una oficina."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Desde que estoy embarazada no me encuentro muy bien, he ido dos veces al\nhospital esta semana y por fin me han dado la baja, ya era hora. No tengo ningunas ganas de\nvolver a mi oficina, ya veremos qué haré en el futuro.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 4,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿Qué ha pasado?",
      "options": [
        {
          "key": "A",
          "text": "Su amiga trabaja en la ambulancia."
        },
        {
          "key": "B",
          "text": "Se ha hecho una herida."
        },
        {
          "key": "C",
          "text": "Se ha caído escalando."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Ayer estaba cocinando y viendo la televisión al mismo tiempo. No sé muy bien\nqué paso, pero me hice un corte bastante grande, me mareé y mi amiga tuvo que llamar a una\nambulancia. Creo que voy a tener que cancelar mi excursión a la montaña del fin del semana,\níbamos a escalar.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 5,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿Qué podemos decir de esta persona?",
      "options": [
        {
          "key": "A",
          "text": "Que es ecologista."
        },
        {
          "key": "B",
          "text": "Que contamina mucho."
        },
        {
          "key": "C",
          "text": "Que es virgen."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Debemos hacer todo lo posible por reciclar y luchar contra la contaminación de\nnuestras ciudades. Si protegemos la naturaleza y conservamos el medio ambiente viviremos en\nun mundo mejor. En este momento lo que más me preocupa es el agujero en la capa de ozono.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 1,
      "q": 6,
      "q_range": [
        1,
        6
      ],
      "answer": "C",
      "prompt": "¿De qué está hablando?",
      "options": [
        {
          "key": "A",
          "text": "Del clima de una región."
        },
        {
          "key": "B",
          "text": "De una receta."
        },
        {
          "key": "C",
          "text": "De cómo cultivar."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Primero hay que plantar las semillas, luego poner un poco de agua y esperar a\nque salgan las raíces y las primeras hojas. Si regamos regularmente y no hace demasiado frío\ndeberían crecer y dar frutos en menos de dos años.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 7,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Ricardo dice que…",
      "options": [
        {
          "key": "A",
          "text": "necesita estudiar muchísimas horas antes de los exámenes."
        },
        {
          "key": "B",
          "text": "aprueba sin problemas."
        },
        {
          "key": "C",
          "text": "es un mal estudiante."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 8,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "¿Cómo es Ricardo?",
      "options": [
        {
          "key": "A",
          "text": "Igual que sus familiares de Argelia."
        },
        {
          "key": "B",
          "text": "Un poco serio."
        },
        {
          "key": "C",
          "text": "Muy moreno."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 9,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Ricardo comenta que…",
      "options": [
        {
          "key": "A",
          "text": "en su dieta tiene pocas proteínas."
        },
        {
          "key": "B",
          "text": "no ha seguido los consejos de sus clases de nutrición."
        },
        {
          "key": "C",
          "text": "lleva una dieta equilibrada."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 10,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "Ricardo dice que…",
      "options": [
        {
          "key": "A",
          "text": "los domingos por la mañana lee un libro sobre nutrición."
        },
        {
          "key": "B",
          "text": "tener resaca es bueno para la salud."
        },
        {
          "key": "C",
          "text": "últimamente bebe demasiado alcohol los sábados por la noche."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 11,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Por la noche…",
      "options": [
        {
          "key": "A",
          "text": "Ricardo lee blogs sobre nutrición."
        },
        {
          "key": "B",
          "text": "cenar fruta es lo más sano."
        },
        {
          "key": "C",
          "text": "se debe comer chorizo o lomo."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 2,
      "q": 12,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "Los familiares de Ricardo…",
      "options": [
        {
          "key": "A",
          "text": "quieren que él adelgace."
        },
        {
          "key": "B",
          "text": "quieren que sea entrenador de un equipo."
        },
        {
          "key": "C",
          "text": "están un poco gordos."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Hola, ¿qué tal? Mi nombre es Ricardo, pero todos me llaman Ricky. Al igual que mi hermana,\nestudio en la universidad de Sevilla, este es nuestro último año, los dos tenemos unas notas\nbastantes buenas. Me considero una persona afortunada, no necesito estudiar muchas horas\npara memorizar o aprender algo. Estoy preparándome para ser profesor de educación física,\nmis asignaturas preferidas son las que hacemos en el exterior, me encanta estar al aire libre.\nSoy español, nací en un pequeño pueblo que está cerca de Cádiz, pero tengo la piel muy oscura\ny la gente me pregunta si soy de Marruecos o de Argelia. El color de piel lo he heredado de mi\npadre, es lo único que tengo suyo, él es una persona seria y trabajadora, por el contrario, yo soy\nmás parecido a mi madre, alegre, hablador y un poco alocado.\nMe encanta la comida basura, sé que debería dar ejemplo y llevar una dieta sana, pero es que\nno puedo resistirme a estos platos pesados de carne que te dejan sin poder ni moverte del\nsofá. El año pasado tuvimos una asignatura sobre nutrición y creo que no sigo ninguna de las\nrecomendaciones que nos daba la profesora. Recuerdo un tema del libro que hablaba sobre la\ninfluencia del alcohol en el cuerpo, desde hace un mes me despierto todos los domingos con\nuna resaca horrible, esto no puede ser bueno.\nHoy en día podemos encontrar información sobre nutrición en muchísimas páginas web. Miles\nde personas te bombardean con sus recomendaciones inútiles. Lo mejor es cenar unas cerezas\no un kiwi, muchísimo más sano que tomar embutidos y fritos por la noche. Esto lo sabe todo el\nmundo, pero… ¿Quién lo hace? Otra cosa que deberíamos controlar es el azúcar, pero es que\ntodo lo que está bueno lleva azúcar.\nHace unos años sufrí un ataque al corazón, el médico dijo que es algo raro a mi edad. Desde\nentonces, mis padres y mi hermana no paran de decirme que tengo que cuidarme y perder algo\nde peso. Me lo recuerdan cada vez que nos vemos, estoy ya un poco cansado de este tema de\nconversación. Mi abuelo era entrenador de fútbol profesional, llegó a entrenar al Atlético de\nMadrid, él es el único que me entiende, me dice siempre que solo hay una vida y que en ella\ntenemos que hacer lo que el corazón nos diga.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 13,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "¿Qué dice la noticia?",
      "options": [
        {
          "key": "A",
          "text": "Que en diciembre se podrá comprar un nuevo programa informático."
        },
        {
          "key": "B",
          "text": "Que se ha descubierto la cura para una enfermedad."
        },
        {
          "key": "C",
          "text": "Que 95% de los científicos de Colorado está en huelga."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 14,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "En la noticia se comenta que…",
      "options": [
        {
          "key": "A",
          "text": "el día internacional del té es en primavera."
        },
        {
          "key": "B",
          "text": "los expertos en té son de Inglaterra."
        },
        {
          "key": "C",
          "text": "las infusiones son buenas para la salud."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 15,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "El piloto…",
      "options": [
        {
          "key": "A",
          "text": "es un militar."
        },
        {
          "key": "B",
          "text": "es muy joven."
        },
        {
          "key": "C",
          "text": "volaba por primera vez."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 16,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "En la noticia se dice que…",
      "options": [
        {
          "key": "A",
          "text": "los oculistas españoles estaban en contra de realizar la conferencia."
        },
        {
          "key": "B",
          "text": "los organizadores del evento son los propietarios de la sala."
        },
        {
          "key": "C",
          "text": "el año pasado hubo bastante público en la conferencia."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 17,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "En los aeropuertos…",
      "options": [
        {
          "key": "A",
          "text": "ha habido problemas después de la decisión del gobierno."
        },
        {
          "key": "B",
          "text": "los niños no pueden estar sin una persona mayor de edad."
        },
        {
          "key": "C",
          "text": "es obligatorio que los empleados lleven gorra."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 3,
      "q": 18,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "Según dice la noticia…",
      "options": [
        {
          "key": "A",
          "text": "los ganaderos han cerrado sus negocios."
        },
        {
          "key": "B",
          "text": "la gente compra menos carne."
        },
        {
          "key": "C",
          "text": "el gobierno ha ofrecido ayudas económicas."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Abrimos el informativo de hoy con una noticia esperanzadora. Un grupo de científicos de la\nUniversidad de Colorado afirma haber encontrado la vacuna contra el sida. El director del\nproyecto asegura que la vacuna tiene una efectividad del 95 % y podría estar en el mercado a\npartir del mes de diciembre.\nHoy se celebra en muchos países el día internacional del té. Lo que para muchos es un ritual\ndiario casi obligatorio, para otros es algo totalmente desconocido. Hemos hablado con varios\nexpertos en nutrición y todos coinciden en lo mismo: una manzanilla, una tila o un poleo menta\npueden tener propiedades que ayudan a nuestro organismo en el día a día.\nPreocupa el estado de salud del piloto del ejército del aire que sufrió el accidente el sábado\npasado durante el desfile del día de la Hispanidad. Su caza se estrelló en un bosque cercano a\nla base desde la que despegó. El soldado tuvo tiempo de saltar en paracaídas, sin embargo, se\nencontraba demasiado cerca del suelo y se rompió varios huesos al caer.\nLa conferencia de oculistas dará comienzo el próximo martes y durará hasta el viernes. En la\nedición anterior se llenó la sala de público que provenía de todas partes de España. Este año los\norganizadores han decidido alquilar una sala mayor para evitar los problemas de aforo y confían\nen que el evento ayude a mejorar a todos los profesionales de este sector.\nYa se pueden ver en los aeropuertos las nuevas medidas de seguridad que aprobó el gobierno\nen enero. Desde hace dos semanas es obligatorio que los menores estén acompañados por un\nadulto en todo momento y queda totalmente prohibido acceder al avión con gorra, sombrero o\ncualquier otro complemento de ropa que permita ocultar la cara. Por el momento, no se han\nproducido incidentes y todo funciona con normalidad.\nLos ganaderos de la región aseguran que, al haber disminuido la demanda de carne en el\núltimo año, las vacas no caben en las granjas. Piden ayudas al gobierno y aseguran que la\nsituación puede obligarles a cerrar sus negocios, lo que aumentará el paro en la región.\nAnuncian que las protestas empezarán la próxima semana.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 19,
      "q_range": [
        19,
        24
      ],
      "answer": "H",
      "prompt": "Persona 19: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: H",
      "cuerpo": "",
      "transcript": "El pasado viernes estuve en la reunión de antiguos alumnos de medicina. Había\nun montón de gente. Cenamos en el restaurante del hotel Milenio, de primero comimos\nensalada de salmón y de segundo había cordero al horno. Lo mejor de todo es que, a la hora\ndel postre, pusieron seis o siete tartas en el centro de la mesa, estaban deliciosas, creo que las\nprobé todas.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 20,
      "q_range": [
        19,
        24
      ],
      "answer": "D",
      "prompt": "Persona 20: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: D",
      "cuerpo": "",
      "transcript": "Fue una pena, había quedado con novio en el gimnasio a las diez y media, pero\ncuando llegué él ya se había ido. Finalmente, estuve una hora haciendo aeróbic y después me\nrelajé en la piscina. Creo que me gusta mucho más bañarme que hacer ejercicio.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 21,
      "q_range": [
        19,
        24
      ],
      "answer": "A",
      "prompt": "Persona 21: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Después de pensarlo bien, he comprado las pastillas para la diarrea que me\nrecomendaron en el centro de salud. En algunos foros de internet leí que no eran muy buenas y\nque podían provocar dolores, pero el enfermero me aseguró que son las mejores que hay\nahora mismo en el mercado y me dijo que en esos foros se escriben muchas tonterías.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 22,
      "q_range": [
        19,
        24
      ],
      "answer": "B",
      "prompt": "Persona 22: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "No sé qué me pasa. Aunque como solamente verduras no consigo perder peso.\nEstoy empezando a pensar que mi cuerpo es así por genética y que no puedo cambiarlo. Solo\ntenemos una vida y si no la disfrutamos creo que perdemos el tiempo que tenemos, para mí\nesta dieta es un sufrimiento.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 23,
      "q_range": [
        19,
        24
      ],
      "answer": "F",
      "prompt": "Persona 23: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: F",
      "cuerpo": "",
      "transcript": "Abrí el buzón y ahí estaba, tenía el sello del hospital, cuando la miré\ndetenidamente me di cuenta de que estaba firmada por el propio doctor Casillas. Por suerte,\neran buenas noticias, no voy a tener que operarme, solo ir a una visita de control en tres\nsemanas.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 4,
      "q": 24,
      "q_range": [
        19,
        24
      ],
      "answer": "E",
      "prompt": "Persona 24: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Va a tomar medicinas."
        },
        {
          "key": "B",
          "text": "No consume carne."
        },
        {
          "key": "C",
          "text": "Pasó bastante tiempo en el hospital."
        },
        {
          "key": "D",
          "text": "No pudo ver a su pareja."
        },
        {
          "key": "E",
          "text": "Hizo un voluntariado."
        },
        {
          "key": "F",
          "text": "Habla de una carta."
        },
        {
          "key": "G",
          "text": "Quiere ser deportista profesional."
        },
        {
          "key": "H",
          "text": "Había muchas personas en este lugar."
        },
        {
          "key": "I",
          "text": "Vive al lado de un famoso."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: E",
      "cuerpo": "",
      "transcript": "Hace unos años estuve colaborando con una ONG, no me pagaban nada, pero\nme daban comida y una habitación para dormir. Me sentía muy bien haciendo mi trabajo y\nayudando a las personas que de verdad lo necesitan. Es algo que le recomiendo a todo el\nmundo, yo lo volvería a hacer.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 25,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "Piensa que el casco es muy bonito.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 26,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "Ha esquiado esta semana.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 27,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "Dice que en un lugar había descuentos.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 28,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "Prefiere no beber alcohol.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 29,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "Da un consejo a la otra persona.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 2,
      "tarea": 5,
      "q": 30,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "Dice que se puede cambiar un producto estropeado.",
      "options": [
        {
          "key": "A",
          "text": "RAFA"
        },
        {
          "key": "B",
          "text": "LOLA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "MUJER: Dígame, ¿quién es?\nHOMBRE: ¡Feliz cumpleaños, Lola! Soy tu primo Rafa. Mira debajo de la mesa del salón, te he\ndejado un regalito. Creo que te va a gustar.\nMUJER: ¡Oh! Son unos esquís, muchísimas gracias. Me encantan.\nHOMBRE: Mira bien, también hay un casco, debe de estar al lado.\nMUJER: Sí, ya lo veo, es precioso.\nHOMBRE: Sé que te gusta cuidarte, quería comprarte unos cosméticos, pero no sabía cuáles\nelegir, así que he optado por tu segunda pasión, los deportes de invierno.\nMUJER: Y has acertado. Pero estos esquís parecen muy buenos, te habrán costado un montón,\nte has pasado, primo.\nHOMBRE: No te preocupes, Lola. La tienda estaba de rebajas. Cuéntame qué tal fue la fiesta\nanoche.\nMUJER: Muy bien, vinieron todos mis amigos y estuvimos aquí en casa.\nHOMBRE: ¿Brindasteis con champán? A mí me encanta.\nMUJER: No, lo probé hace tiempo, pero el alcohol en general tiene un sabor muy fuerte para mí.\nSabes que yo soy más de infusiones.\nHOMBRE: Bueno, pues para llevar una vida sana te recomiendo el canal de YouTube “Sin\nazúcar”, mi novia lo sigue a diario y dice que lo encuentra muy útil.\nMUJER: Muchas gracias, lo voy a buscar ahora mismo.\nHOMBRE: Otra cosa más, los esquíes y el casco tienen garantía, si tienes cualquier problema con\nellos o ves que tienen algún defecto, dímelo, que tengo el ticket de compra en mi casa y vamos\na la tienda para hacer la reclamación.\nMUJER: Perfecto, estoy muy emocionada. Gracias de nuevo.\nHOMBRE: De nada, prima. Espero que nos veamos pronto.\nMUJER: ¡Adiós!",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Rafa (A), a Lola (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 3,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        1
      ],
      "answer": "",
      "prompt": "Tarea 1: Usted ve un anuncio del concurso “Vida sana”. Léalo y escriba su",
      "options": [],
      "type": "writing",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Usted ve un anuncio del concurso “Vida sana”. Léalo y escriba su\nexperiencia (entre 100 y 120 palabras).\nUsted debe:\nsaludar y presentarse;\nhablar sobre su rutina de entrenamiento y dieta;\ndecir qué problema tuvo y cómo lo solucionó;\ndespedirse.\nVIDA SANA\nParticipa en el concurso, cuéntanos lo que haces en tu día a día para estar en forma. Estamos\nbuscando la mejor rutina de entrenamiento y la dieta más sana. ¿Cómo y dónde entrenas? ¿Qué\ncomes? ¡Cuéntanoslo! ¿Tuviste algún problema durante tus entrenamientos? Dinos cómo lo\nsuperaste. Nuestro entrenador personal leerá todos los artículos que escribáis y elegirá el\nmejor. El ganador recibirá un bono de 500€ en equipamiento deportivo en la tienda Maratón.",
      "rango_palabras": "100-120 palabras",
      "modelo_essay": "Estimados responsables del centro deportivo:\n\nMe llamo Carlos Garcia y les escribo porque he visto su anuncio de clases de deporte y me gustaria recibir mas informacion.\n\nActualmente, mi rutina de entrenamiento consiste en salir a correr tres dias a la semana durante unos 40 minutos. En cuanto a mi dieta, intento comer de forma equilibrada: muchas frutas, verduras y proteinas magras, evitando los alimentos procesados y el azucar.\n\nEl ano pasado tuve un problema en la rodilla derecha por correr con un calzado inadecuado. Afortunadamente, lo solucione comprando unas zapatillas de mejor calidad y haciendo ejercicios de fortalecimiento que me recomendo un fisioterapeuta.\n\nMe interesaria saber que horarios tienen disponibles, cuales son los precios y si ofrecen clases de prueba gratuitas. Tambien me gustaria conocer si tienen servicio de entrenador personal.\n\nMuchas gracias por su atencion. Quedo a la espera de su respuesta.\n\nUn cordial saludo,\nCarlos Garcia"
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 3,
      "tarea": 2,
      "q": 2,
      "q_range": [
        2,
        2
      ],
      "answer": "",
      "prompt": "Tarea 2: Elija solo una de las dos opciones que se le ofrecen a continuación y",
      "options": [],
      "type": "writing",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Elija solo una de las dos opciones que se le ofrecen a continuación y\nescriba un texto de entre 130 y 150 palabras:\nOPCIÓN 1\n¿Nos cuentas tu secreto para ponerte guapo o guapa para las fiestas? ¿Cómo te gusta vestirte? ¿Qué\ncosméticos utilizas? ¿Dedicas mucho tiempo a prepararte antes de salir los fines de semana?\nHáblanos sobre tus costumbres de belleza.\nLea el post del blog y escriba su comentario, hable de:\nla ropa que utiliza para salir de fiesta;\nlos cosméticos que más le gustan;\nsu ritual de preparación antes de salir de casa.\nOPCIÓN 2\n¿Debería operarme la nariz? Muchas personas me dicen que tengo una cara muy estrecha y mi nariz\nno me queda bien. Estoy pensando cambiar un poco mi apariencia. ¿Qué pensáis? ¿Debería hacerlo?\nPor favor, sed sinceros.\nSu amiga ha hecho una publicación en las redes sociales, coméntela, usted debe:\ndarle un consejo;\nofrecer su opinión;\ncontar la experiencia de algún conocido con ese tema.",
      "rango_palabras": "130-150 palabras",
      "modelo_essay": "Hola a todos:\n\nHoy quiero compartir con vosotros mi ritual de belleza para salir de fiesta. Soy de esas personas que necesitan al menos dos horas para prepararse antes de una noche especial.\n\nEn primer lugar, me doy una ducha caliente con gel de lavanda para relajarme. Despues, aplico una mascarilla hidratante en el rostro durante 15 minutos mientras elijo la ropa que voy a ponerme.\n\nPara salir de fiesta, me encanta llevar vestidos de color negro porque combinan con todo y estilizan la figura. Mis cosmeticos favoritos son el iluminador para los pomulos, un buen labial rojo de larga duracion y, por supuesto, una mascara de pestanas que de volumen. No puedo salir de casa sin mi perfume de vainilla.\n\nCuando ya estoy lista, me miro al espejo, me hago un selfie para recordar el momento y salgo con la mejor actitud. La belleza empieza por sentirse bien con una misma.\n\n?Cual es vuestro ritual? !Contadme en los comentarios!"
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 4,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        1
      ],
      "answer": "",
      "prompt": "Tarea 1: Le proponemos dos temas con algunas indicaciones para preparar",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Le proponemos dos temas con algunas indicaciones para preparar\nuna exposición oral. Elija uno de ellos. Tendrá que hablar durante 2 o\n3 minutos sobre el tema elegido. El entrevistador no intervendrá en\nesta parte de la prueba. Diferencie las partes de su exposición\n(introducción, desarrollo y conclusión final), ordene y relacione bien las\nideas y justifique sus opiniones y sentimientos.\nOPCIÓN 1\nHable sobre una vez que estuvo en el hospital, comente:\nqué parte del cuerpo le dolía;\nlos motivos por los que tuvo el problema de salud;\ncómo fue su estancia en el hospital;\ncómo acabó todo.\nOPCIÓN 2\nHable sobre los deportes que practicaba de joven, debe:\nhablar sobre cuándo y dónde practicaba estos deportes;\npor qué decidió cambiar de unos a otros o probar nuevos;\ncon quién los practicaba y con qué frecuencia;\ncuál de ellos le gustaba más y por qué.",
      "modelo_response": "Buenos dias, a continuacion voy a hablar sobre mi deporte favorito y por que es tan importante en mi vida.\n\nMi deporte favorito es la natacion. Empece a nadar cuando tenia ocho anos porque mi madre me apunto a clases en la piscina municipal. Al principio no me gustaba mucho, pero poco a poco descubri que el agua me relajaba y me hacia sentir libre.\n\nNado tres veces por semana, normalmente por las mananas antes de ir a trabajar. Para mi, la natacion no es solo un ejercicio fisico, sino tambien una forma de meditacion. Cuando estoy en el agua, todos mis problemas desaparecen y solo me concentro en mi respiracion y en cada brazada.\n\nLos beneficios de la natacion son muchos: mejora la capacidad pulmonar, fortalece todos los musculos del cuerpo y no dana las articulaciones, algo muy importante para personas de todas las edades.\n\nEn mi ciudad, hay una piscina cubierta muy moderna con carriles para nadadores y una zona de spa. Los precios son bastante asequibles y hay descuentos para estudiantes y jubilados.\n\nPara terminar, animo a todo el mundo a probar la natacion. No importa la edad que tengas ni tu condicion fisica, siempre hay un nivel adecuado para ti. !Animate a nadar!",
      "rango_palabras": "2-3 min"
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 4,
      "tarea": 2,
      "q": 2,
      "q_range": [
        2,
        2
      ],
      "answer": "",
      "prompt": "Tarea 2: Cuando haya terminado su exposición (Tarea 1), usted deberá",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Cuando haya terminado su exposición (Tarea 1), usted deberá\nmantener una conversación con el entrevistador sobre el mismo tema\ndurante 3 o 4 minutos.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR:\nSI ELIGIÓ LA OPCIÓN 1\n¿Cómo son los hospitales en el lugar donde vive usted? ¿Funciona bien el sistema de salud?\n¿Conoce a alguien que trabaje en el sector de la salud? ¿Cómo piensa que es este tipo de\ntrabajo?\n¿Qué consejo le puedes dar a una persona que está ingresada mucho tiempo en el\nhospital?\n¿Cuándo era joven le gustaba estar enfermo/a? ¿Por qué?\n¿Hay algún medicamento que odie tomar? ¿Por qué?\nSI ELIGIÓ LA OPCIÓN 2\n¿Le gusta ver deportes en la televisión? ¿Cuáles?\n¿Va a los estadios para ver partidos?\n¿Alguna vez ha viajado para ver o practicar algún deporte? ¿Cómo fue el viaje?\n¿Ha pensado alguna vez en ser deportista profesional? ¿Cree que es un buen trabajo?\n¿Alguna vez ha tenido algún problema mientras practicaba deporte? ¿Qué le sucedió?",
      "modelo_response": "Voy a describir la imagen de las personas haciendo deporte en el parque.\n\nEn la fotografia podemos ver un grupo de cinco personas, tres mujeres y dos hombres, haciendo ejercicio al aire libre en un parque urbano. Todos llevan ropa deportiva de colores vivos y parecen estar en buena forma fisica.\n\nEl lugar es un parque grande con cesped verde y arboles altos al fondo. Se puede ver un camino de tierra y, a lo lejos, algunos edificios de la ciudad. Hay varios bancos de madera y una fuente en el centro.\n\nLas personas estan realizando diferentes ejercicios: dos mujeres estan haciendo estiramientos sobre unas colchonetas, los dos hombres estan corriendo y otra mujer esta bebiendo agua junto a una bicicleta.\n\nImagino que estas personas son companeros de un grupo de entrenamiento que se reunen todos los fines de semana para hacer deporte juntos. Probablemente esten entrenando para alguna carrera popular o simplemente disfrutando de un estilo de vida saludable.\n\nYo tambien he participado en grupos similares. El verano pasado me uni a uno y fue una experiencia increible, conoci a gente maravillosa y mejore mucho mi forma fisica. Recomiendo totalmente hacer deporte en grupo.",
      "rango_palabras": "3-4 min"
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 4,
      "tarea": 3,
      "q": 3,
      "q_range": [
        3,
        3
      ],
      "answer": "",
      "prompt": "Tarea 3: Le proponemos dos fotografías para esta tarea. Elija una, obsérvela y",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Le proponemos dos fotografías para esta tarea. Elija una, obsérvela y\ndescríbala con detalle, durante 1 o 2 minutos, lo que ve en la foto y lo\nque imagina que está ocurriendo.\nEstos son algunos aspectos que puede comentar:\nLas personas: dónde están, cómo son, qué hacen.\nEl lugar en el que se encuentran: cómo es.\nLos objetos: qué objetos hay, dónde están, cómo son.\nQué relación cree que existe entre las personas.\n¿Qué pasa, qué piensan o de qué hablan?\nPosteriormente, el entrevistador le hará algunas preguntas.\nLa duración total de esta tarea es de 2 a 3 minutos.\nOPCIÓN 1\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\n¿Le gusta cocinar? ¿Qué suele hacer en la cocina?\n¿Cuál es su plato preferido? ¿Qué ingredientes lleva?\n¿Sigue alguna dieta especial? ¿Por qué?\nOPCIÓN 2\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\n¿Usted practica deporte? ¿Qué hace para estar en forma? ¿Con qué frecuencia?\n¿Cuáles son los deportes más importantes en su país?\n¿Por qué zonas de su ciudad la gente va a correr? ¿Es peligroso correr por las carreteras?\n¿Ve algún deporte en la televisión o en directo? ¿Cuál?",
      "modelo_response": "Vamos a analizar los datos de la encuesta sobre medios de transporte en las ciudades espanolas.\n\nEl grafico que tenemos delante muestra los medios de transporte mas utilizados en las grandes ciudades de Espana durante el ano 2023. En primer lugar, el coche particular sigue siendo el medio mas usado, con un 42% de los desplazamientos. El transporte publico, que incluye metro y autobus, representa un 35%, mientras que la bicicleta solo alcanza un 8%.\n\nMe sorprende positivamente que caminar sea la tercera opcion, con un 12% de los desplazamientos. Esto indica que muchas personas viven cerca de sus lugares de trabajo o estudio.\n\nComparando estos datos con los de hace diez anos, vemos que el uso del coche ha bajado un 10% y el transporte publico ha subido un 8%. Creo que esto es una tendencia muy positiva para el medio ambiente.\n\nEn mi opinion, las ciudades deberian seguir invirtiendo en carriles bici y en mejorar el transporte publico. Ademas, las campanas de concienciacion sobre la contaminacion estan dando resultados. Si todos ponemos un poco de nuestra parte, podemos conseguir ciudades mas limpias y habitables.",
      "rango_palabras": "2-3 min"
    },
    {
      "modelo": "nuevo_m3",
      "modelo_name": "Nuevo DELE B1 Modelo 3",
      "prueba": 4,
      "tarea": 4,
      "q": 4,
      "q_range": [
        4,
        4
      ],
      "answer": "",
      "prompt": "Tarea 4: Usted debe dialogar con el entrevistador en una situación simulada",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Usted debe dialogar con el entrevistador en una situación simulada\ndurante dos o tres minutos. La situación tiene relación con la imagen\nelegida en la tarea 3. La duración de esta tarea es de 2 a 3 minutos.\nSI ELIGIÓ LA OPCIÓN 1\nUsted está cocinando algo con otra persona, sin embargo, ha olvidado comprar uno de los\ningredientes. Discuta sobre cómo cocinar el plato sin este ingrediente\nDurante la conversación usted debe:\nexplicar dónde compró todo y por qué olvidó comprar el ingrediente que falta;\nhablar sobre el plato que querían cocinar;\ndar alguna alternativa para cocinar el plato;\ncontar alguna historia de otra vez que le sucedió algo similar.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\nOh, ¿cómo has podido olvidarlo? ¿Qué hacías en la tienda?\n¿Crees que podremos hacer esto de otra forma?\nNo creo que salga bien, ¿no piensas que sería mejor pedir una pizza y cocinar esto otro día?\n¿Alguna vez te ha pasado algo así?\nSI ELIGIÓ LA OPCIÓN 2\nUsted ha decidido hacer deporte para estar en forma, pero quiere hacerlo en compañía de\nalguien. Hable con un amigo para ver si quiere hacer deporte con usted.\nDurante la conversación usted debe:\nexplicar por qué quiere hacer deporte ahora;\ndecir qué deporte le gustaría practicar y con qué frecuencia;\nhablar sobre los aspectos positivos de este deporte;\nofrecer información sobre el lugar donde se puede hacer y los costes que tiene.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\n¿Por qué has pensado en este momento en hacer deporte?\nA mí el único deporte que me gusta es el ajedrez, ¿qué te parece?\n¿Qué deporte podríamos hacer juntos? ¿Por qué?\n¿Dónde se puede practicar? ¿Cuánto cuesta?",
      "modelo_response": "ENTREVISTADOR: Oh, ¿cómo has podido olvidarlo? ¿Qué hacías en la tienda?\nYO: Pues estaba mirando la receta en el móvil y, al llegar a la caja, me puse a hablar con una vecina. Compré las verduras, el pollo y el arroz, pero se me quedó el curry en la estantería.\nENTREVISTADOR: ¿Qué plato querían cocinar?\nYO: Queríamos hacer un arroz con verduras y pollo al curry, para cuatro personas. Era un plato un poco especial, porque hacía tiempo que no cocinábamos juntos.\nENTREVISTADOR: ¿Crees que podremos hacer esto de otra forma?\nYO: Sí, claro. Podemos hacer el arroz igual, pero con las especias que ya tenemos: pimentón, comino y un poco de cúrcuma. Cambiará el sabor, aunque quedará bien. Si no te convence, también podemos preparar una salsa de tomate y albahaca.\nENTREVISTADOR: No creo que salga bien, ¿no piensas que sería mejor pedir una pizza y cocinar esto otro día?\nYO: Te entiendo, pero la verdura ya está cortada y sería una pena tirarla. Probemos hoy con las especias y, si al final no nos gusta, el sábado compramos el curry y lo repetimos. ¿Te parece bien?\nENTREVISTADOR: ¿Alguna vez te ha pasado algo así?\nYO: Sí, una vez preparé una tarta y me di cuenta al final de que no tenía levadura. La hice sin ella y quedó más plana, pero nos la comimos igual.",
      "rango_palabras": "2-3 min"
    }
  ]
};
