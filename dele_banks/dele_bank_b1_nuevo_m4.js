window.DELE_BANK_B1_NUEVO_M4 = {
  "version": "2.0",
  "level": "B1",
  "source": "Nuevo DELE B1 Modelo 4 — Ramón Díez Galán. Reconstrucción íntegra desde el PDF: lectura 30 + auditiva 30 (con transcripciones) + escritura 2 + expresión oral 4 (incluida la Tarea 4 de diálogo).",
  "language": "es-ES",
  "syllabus": {
    "level": "B1",
    "duration_minutes": {
      "reading": 70,
      "listening": 40,
      "writing": 60,
      "speaking_prep": 15,
      "speaking_test": 15
    }
  },
  "items": [
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        6
      ],
      "answer": "E",
      "prompt": "¿A qué texto corresponde María (persona 1)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: E",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 2,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿A qué texto corresponde Raúl (persona 2)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 3,
      "q_range": [
        1,
        6
      ],
      "answer": "G",
      "prompt": "¿A qué texto corresponde Ana (persona 3)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: G",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 4,
      "q_range": [
        1,
        6
      ],
      "answer": "I",
      "prompt": "¿A qué texto corresponde Ramón (persona 4)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: I",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 5,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿A qué texto corresponde Pedro (persona 5)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 1,
      "q": 6,
      "q_range": [
        1,
        6
      ],
      "answer": "H",
      "prompt": "¿A qué texto corresponde Luisa (persona 6)?",
      "options": [
        {
          "key": "A",
          "text": "MUNDO.COM. Si quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar."
        },
        {
          "key": "B",
          "text": "URGENTE. ¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías."
        },
        {
          "key": "C",
          "text": "CAMBIOS EN EL HORARIO. Las clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase."
        },
        {
          "key": "D",
          "text": "ACCIDENTE. Como bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos."
        },
        {
          "key": "E",
          "text": "AVISO. Te recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB."
        },
        {
          "key": "F",
          "text": "COMPROBAR E-MAIL. El departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija."
        },
        {
          "key": "G",
          "text": "MENSAJE DEL DIRECTOR REENVIADO. Si alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca."
        },
        {
          "key": "H",
          "text": "RECORDATORIO. Se recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo."
        },
        {
          "key": "I",
          "text": "RECUPERACIÓN. El día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112"
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: H",
      "cuerpo": "PERSONAS:\n1. María: Estoy súper agobiada, no puedo más, tengo que preparar una presentación con los resultados del mes para los jefes de la empresa. Debo utilizar el programa PowerPoint o alguno similar.\n2. Raúl: Tengo un problema, por mi culpa, mi empresa va a tener que pagar una compensación muy grande a otra compañía. Adiós a mis opciones de promoción.\n3. Ana: Para matricularme este año en la universidad voy a necesitar una ayuda económica. Han subido muchísimo los pagos mensuales que tenemos que hacer.\n4. Ramón: Mi hijo ha suspendido todos los controles que ha hecho este año. El jueves tiene su última oportunidad para no quedarse atrás. Quiero que el próximo curso continúe con sus compañeros.\n5. Pedro: Estoy muy emocionado, acabo de terminar de hacer mi primer currículum y ahora voy a presentárselo a diferentes compañías del sector turístico.\n6. Luisa: Me parece increíble que en mi nuevo puesto de trabajo nos obliguen a buscar y pagar nosotros mismos el equipamiento de seguridad, la empresa debería encargarse de esto. 111\n\nTEXTOS:\nA. MUNDO.COM\nSi quieres formar parte de un equipo joven y dinámico esta oferta de empleo es para ti. Buscamos gente que ayude a expandir nuestra empresa online a diferentes partes del mundo. No se requiere experiencia previa, pero sí ilusión y ganas de trabajar.\n\nB. URGENTE\n¿En qué estabas pensando? Te has equivocado al introducir los datos de la factura en el ordenador. A partir de ahora, la empresa productora nos va a subir los precios muchísimo. Puedes olvidarte del puesto de subdirector de departamento que pedías.\n\nC. CAMBIOS EN EL HORARIO\nLas clases de historia del profesor Martínez pasarán a los martes a la misma hora, mientras tanto, los miércoles tendremos una hora de tutoría en la que los alumnos podrán preguntar sus dudas y resolver los conflictos internos de la clase.\n\nD. ACCIDENTE\nComo bien sabéis todos, el pasado lunes por la mañana tuvo lugar un desafortunado accidente laboral en nuestra empresa. Matías, un empleado del departamento de recursos humanos se cayó por las escaleras y se rompió la rodilla derecha. Tened cuidado al caminar por los pasillos.\n\nE. AVISO\nTe recuerdo que tienes la exposición del informe económico mensual el próximo martes 3 de mayo. Nos vemos en la sala 23, junto al comedor, allí podremos ver mejor las diapositivas con el proyector. Tráelo todo preparado en un USB.\n\nF. COMPROBAR E-MAIL\nEl departamento de informática ha detectado un error con los correos electrónicos. Al introducir la contraseña algunos usuarios no podían escribir más de cinco letras. Si alguien tiene este problema debe hablar con Juan para que lo corrija.\n\nG. MENSAJE DEL DIRECTOR REENVIADO\nSi alguien de vosotros está pensando en pedir una beca para este curso debe rellenar el formulario que adjunto en este email y entregarlo en la secretaría antes del viernes. El rector todavía no ha decidido cuál será la cantidad que recibirán los estudiantes que obtengan la beca.\n\nH. RECORDATORIO\nSe recuerda a todos los empleados de la planta de producción que deben comprar unas botas de trabajo y un casco homologado. Será obligatorio llevar botas y casco en toda la zona de producción y en el almacén. De este modo, nos adaptaremos a las nuevas normas del Ministerio de Trabajo.\n\nI. RECUPERACIÓN\nEl día 13 tendremos el test de recuperación para todas aquellas personas que no han aprobado alguna asignatura. Recordad, es muy importante, si en este test se obtiene una mala nota, el alumno tendrá que repetir el curso. 112",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer seis textos en los que unas personas hablan sobre su vida y diez textos con mensajes. Relacione a las personas (1-6) con los textos (A-I). HAY TRES TEXTOS QUE NO DEBE RELACIONAR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 7,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "Según el texto…",
      "options": [
        {
          "key": "A",
          "text": "puedes necesitar mucho tiempo para encontrar tu primer trabajo."
        },
        {
          "key": "B",
          "text": "debes realizar tu currículum antes de terminar tus estudios."
        },
        {
          "key": "C",
          "text": "encontrar trabajo tras los estudios es sencillo."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 8,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "El currículum…",
      "options": [
        {
          "key": "A",
          "text": "siempre debe estar escrito en una lengua extranjera."
        },
        {
          "key": "B",
          "text": "debe ser igual al de otros candidatos."
        },
        {
          "key": "C",
          "text": "se hace para poder tener una entrevista de trabajo."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 9,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "El texto dice que, gracias a la Universidad,…",
      "options": [
        {
          "key": "A",
          "text": "todos los alumnos hacen prácticas o voluntariados."
        },
        {
          "key": "B",
          "text": "cualquier empresa aceptará tener entrevistas de trabajo contigo."
        },
        {
          "key": "C",
          "text": "algunos estudiantes pueden obtener experiencia con becas y programas especiales de prácticas."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 10,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "El texto comenta que…",
      "options": [
        {
          "key": "A",
          "text": "puedes recibir dinero por realizar algunas prácticas en empresas."
        },
        {
          "key": "B",
          "text": "las prácticas siempre deben tener un sueldo fijo."
        },
        {
          "key": "C",
          "text": "las empresas prefieren a los trabajadores sin experiencia."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 11,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "El texto dice que en las redes sociales…",
      "options": [
        {
          "key": "A",
          "text": "es mejor no tener una foto personal."
        },
        {
          "key": "B",
          "text": "hay prácticas online a buen precio."
        },
        {
          "key": "C",
          "text": "debes ofrecer una buena imagen."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 2,
      "q": 12,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "El autor del texto recomienda…",
      "options": [
        {
          "key": "A",
          "text": "crear una pequeña empresa."
        },
        {
          "key": "B",
          "text": "aprender diferentes lenguas."
        },
        {
          "key": "C",
          "text": "trabajar en lo primero que se encuentre."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "Cómo buscar tu primer empleo tras finalizar la carrera\nEncontrar empleo tras finalizar la carrera es complicado, no todo el mundo puede acceder a un\npuesto de trabajo nada más finalizar los estudios. Menos aún, trabajar de lo que has estudiado.\nEl proceso de buscar tu primer empleo puede llevar mucho tiempo. No te pongas nervioso,\ntodo llega a su momento.\nUn buen currículum siempre destacará sobre el resto. Tienes que crear un currículum perfecto,\nno lo desarrolles a la ligera para enviarlo cuanto antes a las ofertas de empleo que van\napareciendo. Dedica el tiempo que se merece a crear un currículum que impresione.\nEl objetivo principal del currículum a día de hoy es\nconseguir una entrevista de trabajo. Muchos\nconfunden la función del currículum creyendo que\nun currículum es para encontrar trabajo. No, no es\nasí. En un currículum debes captar la atención de la\npersona que está buscando empleados. Debe ser\natractivo, que se diferencie del resto.\nLo ideal es que te pongas en contacto con la Universidad para que te orienten sobre los\ndiferentes programas a los que puedes tener acceso tras acabar tus estudios. No sólo hay\noportunidades para realizar prácticas. También hay oportunidades para obtener una beca o\nrealizar un voluntariado y así tener experiencia laboral. Lógicamente, no pueden entrar todos\nlos estudiantes universitarios a estos puestos.\nSi necesitas experiencia para acceder a un puesto de empleo en la empresa, no dudes en\nbuscar la forma de obtener experiencia y, en un futuro, optar a este puesto de trabajo. A veces\nhay que invertir, en este caso tiempo, para conseguir un objetivo, trabajo. Hay diversas formas\npara adquirir experiencia profesional. A día de hoy aparecen muchas oportunidades para\nrealizar prácticas en empresas, muchas de ellas, pagadas.\nHay prácticas a nivel nacional como a nivel internacional. Las hay para todo tipo de\nprofesionales. Algunas convocatorias tienen fecha y otras están permanentes durante todo el\naño. Sólo tienes que buscar la que mejor se adapte a tu perfil.\nRepasa tus redes sociales y limpia todo aquello que pueda perjudicar a tu imagen para ser\ncontratado. Hay personas que, para acceder a un puesto de empleo concreto, llegan a cerrar\ntodas sus redes sociales. Usa una foto profesional. La foto será la primera imagen visual que\ndes al mundo.\nNunca dejes de aprender. La correcta formación te abrirá las puertas no sólo a tu primer\nempleo tras finalizar la carrera, también a los empleos a los que quieras optar después. Los\nidiomas serán otro de los factores claves a la hora de acceder a un primer empleo. No te\nquedes sólo con una lengua. Ahora, sólo queda ponerse a buscar tu primer empleo.\nAdaptado de: www.trabajarporelmundo.org",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer un texto sobre cómo buscar el primer empleo. Después, debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 13,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "¿Quién dice que se dedica al sector de la producción?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 14,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "¿Qué persona dice que en la actualidad tiene una hipoteca?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 15,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "¿Quién dice que tiene muchas ganas de encontrar un trabajo?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 16,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "¿Quién comenta que obtuvo un ascenso en su empleo?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 17,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "¿Quién valora sobre todo la seguridad y la 17 posibilidad de crecer dentro de la empresa?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 3,
      "q": 18,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "¿Qué persona dice que no le parece bien 18 que haya impuestos en las carreras universitarias?",
      "options": [
        {
          "key": "A",
          "text": "ADA"
        },
        {
          "key": "B",
          "text": "EDU"
        },
        {
          "key": "C",
          "text": "MAR"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "TEXTOS:\nA. ADA\nEstudié enfermería en una universidad privada en Valencia. Recuerdo que mis padres pagaron muchísimo, no entiendo por qué, además de las tasas, debemos pagar el IVA por los estudios, me parece un robo. Lo único bueno de aquella época es que conocí a mi marido, los dos trabajamos en el mismo centro de salud. Este año es muy importante en nuestras vidas, hace unos meses decidimos pedir un crédito para poder comprar una casa preciosa con jardín y piscina, estamos ya casi listos para mudarnos. Espero que el sector de la salud no tenga problemas, porque como mi marido y yo perdamos el trabajo no podremos devolver el crédito y perderemos la casa.\n\nB. EDU\nUna vida perfectamente planificada por mi padre, eso es lo que he tenido yo. Me fui con una beca a Inglaterra y cursé los estudios de mecánica industrial porque mi padre así lo quiso. Mi familia tiene una de las mayores fábricas de materiales metálicos para la construcción que hay en Europa. Somos líderes en el mercado y nos buscan clientes de todo el mundo. Mi futuro estaba decidido, cuando volví a España empecé a trabajar en la fábrica con un puesto sin demasiada responsabilidad, a los pocos meses pasé a ser el jefe del departamento de mecánica, ahora ya siento que estaré en esta empresa el resto de mi vida.\n\nC. MAR\nHace apenas tres meses que acabé la carrera universitaria de derecho laboral. Envié una carta de presentación a varias empresas y estoy esperando su respuesta. Tengo muchísima motivación y mi disponibilidad es total, me da igual tener que hacer horario nocturno. En mi opinión, dentro de una empresa es muy importante tener opciones reales de promoción y un buen sindicato que apoye a los empleados. Por eso, quiero centrarme en las grandes corporaciones, como hizo mi hermana, ella ahora es la directora de contabilidad de una empresa que produce comida para animales y exporta sus productos a varios países. 117",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Usted va a leer tres textos en los que unas personas nos hablan de sus estudios y trabajos. Relacione las preguntas (13-18) con los textos (A, B o C)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 19,
      "q_range": [
        19,
        24
      ],
      "answer": "B",
      "prompt": "Pregunta 19",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 20,
      "q_range": [
        19,
        24
      ],
      "answer": "E",
      "prompt": "Pregunta 20",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: E",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 21,
      "q_range": [
        19,
        24
      ],
      "answer": "H",
      "prompt": "Pregunta 21",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: H",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 22,
      "q_range": [
        19,
        24
      ],
      "answer": "C",
      "prompt": "Pregunta 22",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 23,
      "q_range": [
        19,
        24
      ],
      "answer": "A",
      "prompt": "Pregunta 23",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 4,
      "q": 24,
      "q_range": [
        19,
        24
      ],
      "answer": "G",
      "prompt": "Pregunta 24",
      "options": [
        {
          "key": "A",
          "text": "Marx y Engels en su “Manifiesto comunista” de 1848, realizaron una denuncia al trabajo de la sociedad capitalista, que formaría las bases del comunismo soviético"
        },
        {
          "key": "B",
          "text": "El esclavo trabajaba para su dueño, que había invertido dinero en su compra, o había nacido de una esclava suya"
        },
        {
          "key": "C",
          "text": "Junto al trabajo asalariado, y a la Revolución Industrial, creció la explotación de los trabajadores de fábricas"
        },
        {
          "key": "D",
          "text": "El actual puesto de director de recursos humanos proviene precisamente de este empleo"
        },
        {
          "key": "E",
          "text": "Así, en la Edad Media, existía una distribución de funciones entre quienes peleaban para defender todo el reino en guerras (los nobles)"
        },
        {
          "key": "F",
          "text": "Varios de estos países se sumaron a las protestas y la situación se descontroló"
        },
        {
          "key": "G",
          "text": "Al firmarse el Tratado de Versalles en 1919 como culminación de este conflicto mundial"
        },
        {
          "key": "H",
          "text": "En la etapa medieval el trabajo estaba a cargo de los vasallos, siendo los siervos de los ricos 119"
        }
      ],
      "type": "choice8",
      "_v5": true,
      "explanation": "Respuesta correcta: G",
      "cuerpo": "El trabajo en la historia\nEl trabajo que predominó en la Edad Antigua fue bajo la forma de esclavitud, donde quien lo\nrealizaba no tenía derechos y proporcionaba un alto beneficio económico, ya que no tenía\nningún tipo de salario. …………(19)…………... Esta forma persistió hasta el siglo XIX. Había algunos\ntrabajos en esta etapa que eran más prestigiosos que otros, como el de los escribas, los\nescultores o constructores de templos, cuyas actividades estuvieron protegidas en el Código de\nHammurabi.\nLas actividades agrícolas si bien fueron fundamentales para la economía antigua y medieval, no\ncolocaron a quienes las ejercían en un lugar de privilegio, sino más bien era considerada como\nuna actividad que sustentaba a las clases más calificadas. …………(20)…………..., los que rezaban\npara el bien de todos (el clero) y los que trabajaban para todos (los campesinos) que\nalimentaban a aquellos que no producían.\n…………(21)…………..., eran los últimos de la cadena, estaban tan unidos a la tierra que trabajaban,\nque se vendían junto a ella. Las actividades independientes se reglaban a través de los gremios.\nEn la Edad Moderna, cobró auge la\nactividad comercial, con las nuevas\ntecnologías que dieron origen al\ndespegue de la producción en cadena, y\nal nacimiento del capitalismo. …………\n(22)…………..., sin leyes protectoras, por lo\nque sus condiciones de trabajo eran\ninsalubres, sin protección de la\nmaternidad, ni de la familia, ni del trabajo\nde menores; sin descansos y con\njornadas laborales agotadoras.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el siguiente texto, del que se han extraído seis fragmentos. A continuación lea los ocho fragmentos propuestos (A-H) y decida en qué lugar del texto (19-24) hay que colocar cada uno de ellos. HAY DOS FRAGMENTOS QUE NO TIENE QUE ELEGIR."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 25,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "…del lunes 1 de marzo entran en vigor las _____ normas: Queda totalmente pr…",
      "options": [
        {
          "key": "A",
          "text": "antes"
        },
        {
          "key": "B",
          "text": "siguientes"
        },
        {
          "key": "C",
          "text": "después"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 26,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "…Queda totalmente prohibido comer y beber _____ de la sala de estudio. Hemo…",
      "options": [
        {
          "key": "A",
          "text": "dentro"
        },
        {
          "key": "B",
          "text": "entro"
        },
        {
          "key": "C",
          "text": "debajo"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 27,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "…uarios. No se permite tener los teléfonos _____ , incluso si están en modo s…",
      "options": [
        {
          "key": "A",
          "text": "rotos"
        },
        {
          "key": "B",
          "text": "prestados"
        },
        {
          "key": "C",
          "text": "encendidos"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 28,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "…e los estudiantes estén pendientes de las _____ sociales. Los viernes y sáb…",
      "options": [
        {
          "key": "A",
          "text": "redes"
        },
        {
          "key": "B",
          "text": "red"
        },
        {
          "key": "C",
          "text": "olas"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 29,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "…ches de los fines de semana. El rector ha _____ esta decisión puesto que ha…",
      "options": [
        {
          "key": "A",
          "text": "tomado"
        },
        {
          "key": "B",
          "text": "tocado"
        },
        {
          "key": "C",
          "text": "navegado"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 1,
      "tarea": 5,
      "q": 30,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "…o tipo de material de estudio no se puede _____ ocupando espacio sobre la m…",
      "options": [
        {
          "key": "A",
          "text": "cambiar"
        },
        {
          "key": "B",
          "text": "probar"
        },
        {
          "key": "C",
          "text": "dejar"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "NUEVAS NORMAS DE LA SALA DE ESTUDIO\nA partir del lunes 1 de marzo entran en vigor las …..25….. normas:\nQueda totalmente prohibido comer y beber …..26….. de la sala de estudio. Hemos decidido\nadoptar esta medida tras las numerosas quejas de varios usuarios.\nNo se permite tener los teléfonos …..27….., incluso si están en modo silencioso. La sala de\nestudio debe ser un lugar de concentración y tranquilidad, esto no es compatible con que\nlos estudiantes estén pendientes de las …..28….. sociales.\nLos viernes y sábados por la noche se debe abandonar la sala de estudio antes de las 21:00.\nTodo el recinto de la Universidad permanecerá cerrado durante las noches de los fines de\nsemana. El rector ha …..29….. esta decisión puesto que había algunos individuos que no\nsabían comportarse.\nLos libros, mochilas, estuches y cualquier otro tipo de material de estudio no se puede\n…..30….. ocupando espacio sobre la mesa si el estudiante no está en la sala, especialmente\nen temporada de exámenes.",
      "transcript": "",
      "audio": "",
      "instrucciones": "INSTRUCCIONES: Lea el texto y rellene los huecos (25-30) con la opción correcta (a / b / c)."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿Qué sabemos de esta persona?",
      "options": [
        {
          "key": "A",
          "text": "Que no vive en el centro de la ciudad."
        },
        {
          "key": "B",
          "text": "Que no tiene animales en casa."
        },
        {
          "key": "C",
          "text": "Que no tiene prisa por las mañanas."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Desde que hemos cambiado de oficina lo tengo mucho más difícil por las\nmañanas, ahora trabajo en las afueras de la ciudad y tengo que levantarme a las seis de la\nmañana, ponerle comida al conejo y salir corriendo de casa.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 2,
      "q_range": [
        1,
        6
      ],
      "answer": "C",
      "prompt": "¿Qué deben hacer los trabajadores?",
      "options": [
        {
          "key": "A",
          "text": "Fregar el suelo de la cafetería."
        },
        {
          "key": "B",
          "text": "Traer el café de sus casas."
        },
        {
          "key": "C",
          "text": "Limpiar los utensilios de la cocina de la empresa."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Por favor, por respeto al resto de empleados, fregad después de utilizar las\ntazas, cucharillas y vasos. Por su parte, la empresa se compromete a comprar un café de mejor\ncalidad. Entre todos haremos que el día a día sea más agradable.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 3,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿De qué está hablando?",
      "options": [
        {
          "key": "A",
          "text": "De una reunión con un cliente."
        },
        {
          "key": "B",
          "text": "De cómo acceder a un programa informático."
        },
        {
          "key": "C",
          "text": "De un nuevo despacho."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Para acceder, debes introducir tu nombre de usuario y tu contraseña en la\npágina principal y hacer clic en el botón naranja. Una vez dentro, ya podrás descargar las\nfacturas del cliente. Si necesitas ayuda ven a mi despacho.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 4,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿Qué es este lugar?",
      "options": [
        {
          "key": "A",
          "text": "Es un campus."
        },
        {
          "key": "B",
          "text": "Es una guardería."
        },
        {
          "key": "C",
          "text": "Es una cárcel."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Todos los padres deben recoger a sus hijos antes de las cinco de la tarde,\nespecialmente a los de menos de dos años. Las cuidadoras no pueden quedarse más tiempo\nhaciendo su trabajo del que pone en su contrato. Por favor, entended la situación.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 5,
      "q_range": [
        1,
        6
      ],
      "answer": "A",
      "prompt": "¿Qué quiere hacer la persona que habla?",
      "options": [
        {
          "key": "A",
          "text": "Hablar con Pedro de algo del trabajo."
        },
        {
          "key": "B",
          "text": "Verse con Pedro después de mucho tiempo."
        },
        {
          "key": "C",
          "text": "Tomar un café con su amigo Pedro."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Hola Pedro, me gustaría charlar contigo sobre lo que pasó el otro día en la\nfábrica. El lugar y la hora me dan igual, podemos vernos, por ejemplo, en la cafetería de la zona\nnorte a las seis de la tarde.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 1,
      "q": 6,
      "q_range": [
        1,
        6
      ],
      "answer": "B",
      "prompt": "¿Qué necesita esta persona?",
      "options": [
        {
          "key": "A",
          "text": "Una impresora."
        },
        {
          "key": "B",
          "text": "Papel para imprimir."
        },
        {
          "key": "C",
          "text": "Ver al jefe en persona."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Por favor, dile al jefe que compre tres o cuatro paquetes de folios, ya casi no\nnos quedan. Y que lo haga rápido, los vamos a necesitar ahora que nos ha dicho que tenemos\nque imprimir todas las facturas.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis mensajes. Escuche cada mensaje dos veces. Después debe contestar a las preguntas (1-6). Seleccione la opción correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 7,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "Andrea dice que…",
      "options": [
        {
          "key": "A",
          "text": "gana mucho dinero en Valencia."
        },
        {
          "key": "B",
          "text": "trabaja en la ciudad donde nació."
        },
        {
          "key": "C",
          "text": "se dedica a la investigación médica."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 8,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "¿Qué sabemos sobre los padres de Andrea?",
      "options": [
        {
          "key": "A",
          "text": "Que trabajan en la administración pública."
        },
        {
          "key": "B",
          "text": "Que trabajan en diferentes lugares."
        },
        {
          "key": "C",
          "text": "Que tienen su propia empresa."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 9,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Andrea comenta que…",
      "options": [
        {
          "key": "A",
          "text": "tiene buenos recuerdos de sus jefes."
        },
        {
          "key": "B",
          "text": "durante sus estudios tuvo trabajos malos."
        },
        {
          "key": "C",
          "text": "sus padres le ayudaron económicamente cuando estaba en la universidad."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 10,
      "q_range": [
        7,
        12
      ],
      "answer": "A",
      "prompt": "En cuanto a sus hábitos alimenticios, Andrea dice que…",
      "options": [
        {
          "key": "A",
          "text": "no hace caso de los consejos que le dan otras personas para freír."
        },
        {
          "key": "B",
          "text": "le gusta probar diferentes tipos de aceites."
        },
        {
          "key": "C",
          "text": "en su casa nunca hay huevos."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 11,
      "q_range": [
        7,
        12
      ],
      "answer": "B",
      "prompt": "Andrea dice que durante su trabajo…",
      "options": [
        {
          "key": "A",
          "text": "habla con sus padres."
        },
        {
          "key": "B",
          "text": "para varias veces para comer."
        },
        {
          "key": "C",
          "text": "utiliza las redes sociales."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 2,
      "q": 12,
      "q_range": [
        7,
        12
      ],
      "answer": "C",
      "prompt": "El jefe de Andrea…",
      "options": [
        {
          "key": "A",
          "text": "siempre ha sido una persona muy agradable."
        },
        {
          "key": "B",
          "text": "empezó trabajando de recepcionista."
        },
        {
          "key": "C",
          "text": "tiene una novia nueva."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Me llamo Andrea, soy de Zaragoza, pero desde hace cinco años vivo en Valencia. Me mudé aquí\npor trabajo, soy científica, trabajo en un laboratorio. Mi trabajo consiste en buscar nuevas\nmedicinas y vacunas, me encanta lo que hago a pesar de que el sueldo no sea muy alto, me\nsiento útil. Me parece increíble que no se pague más a los científicos en este país, sin nuestro\ntrabajo la sociedad no avanzaría y seguiríamos sin tener soluciones para las enfermedades más\nbásicas.\nValencia no está mal, pero echo muchísimo de menos la ciudad donde nací, sobre todo porque\nallí vive casi toda mi familia. Mis padres son funcionarios, trabajan en la oficina donde la gente se\nhace el DNI, se conocieron allí hace ya más de veinticinco años y desde entonces trabajan\njuntos. Quizás no sea el lugar más romántico del mundo para conocerse, pero ellos son muy\nfelices.\nRecuerdo que durante mis estudios quería tener algo de dinero, por eso estuve trabajando en\nvarias empresas, no tengo muy buenos recuerdos de aquellos empleos. En la mayoría de los\ncasos, los jefes se aprovechaban de que los trabajadores éramos jóvenes y nos ofrecían unas\ncondiciones pésimas, me pone de muy mal humor pensar en ello. Por suerte todo aquello ya\npasó. Acabé mi carrera sin tener que pedir dinero a mis padres, me siento orgullosa de mí\nmisma.\nEn la cocina soy una persona muy específica, tengo algunas manías. Por ejemplo, me encantan\nlos huevos duros, pero nunca me como la yema, siempre hago el mismo ritual: quito la cáscara\ndel huevo, aparto la yema y me como solamente la clara. Con el aceite también soy bastante\nespecial, solo utilizo aceite de oliva, hace más de diez años que no pruebo otro. Mucha gente\nme ha dicho que el aceite de girasol es mejor para freír, pero cuando se me mete algo en la\ncabeza no hay quien me lo quite, así que seguiré haciendo mis patatas fritas con aceite de oliva.\nEn el trabajo tenemos muchísima libertad para hacer lo que queramos, eso está muy bien.\nCuando me entra el hambre, que suele ser con bastante frecuencia, puedo hacer un descanso\nde diez minutos y tomar algo. Mi jefe, que antes era una persona muy arrogante e impaciente,\núltimamente ha cambiado mucho y está de un humor increíble. La gente comenta que está\nsaliendo con una de las recepcionistas del turno de tarde y está totalmente enamorado.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una audición dos veces. Después debe contestar a las preguntas (7-12). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 13,
      "q_range": [
        13,
        18
      ],
      "answer": "B",
      "prompt": "Amanda López…",
      "options": [
        {
          "key": "A",
          "text": "no trabajaba los sábados."
        },
        {
          "key": "B",
          "text": "trabajaba cuidando niños."
        },
        {
          "key": "C",
          "text": "trabajaba en una oficina."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 14,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "El presidente…",
      "options": [
        {
          "key": "A",
          "text": "dice que pagará todo en marzo."
        },
        {
          "key": "B",
          "text": "dice que los jubilados ya han recibido el dinero de las ayudas."
        },
        {
          "key": "C",
          "text": "dice que todavía no ha recibido el dinero de la Unión Europea."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 15,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "¿Qué ha pasado con IBERLUZ?",
      "options": [
        {
          "key": "A",
          "text": "Cerró hace 5 años."
        },
        {
          "key": "B",
          "text": "El presidente ha cambiado."
        },
        {
          "key": "C",
          "text": "Había errores en las facturas."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 16,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "En la Universidad…",
      "options": [
        {
          "key": "A",
          "text": "va a haber hierba."
        },
        {
          "key": "B",
          "text": "se van a renovar las aulas el próximo curso."
        },
        {
          "key": "C",
          "text": "hay obras en este momento."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 17,
      "q_range": [
        13,
        18
      ],
      "answer": "A",
      "prompt": "El centro social…",
      "options": [
        {
          "key": "A",
          "text": "es un lugar de ocio."
        },
        {
          "key": "B",
          "text": "está cerca del centro de la ciudad."
        },
        {
          "key": "C",
          "text": "es para gente joven."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 3,
      "q": 18,
      "q_range": [
        13,
        18
      ],
      "answer": "C",
      "prompt": "Al final del informativo…",
      "options": [
        {
          "key": "A",
          "text": "comentan que se paga para instalar la aplicación."
        },
        {
          "key": "B",
          "text": "dicen que en el futuro solo se podrán ver las noticias en la aplicación."
        },
        {
          "key": "C",
          "text": "recomiendan a los espectadores bajar su aplicación."
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "Bienvenidos a “Informe semanal”. Hoy empezamos hablando de un misterio que sigue sin\nresolverse en Zaragoza. La policía todavía no ha podido encontrar a la joven Amanda Sánchez\nque lleva desaparecida desde el sábado pasado. Lo último que se sabe de Amanda es que\nestaba trabajando de canguro para una familia durante el fin de semana. Los padres de la joven\npiden ayuda a todos los vecinos de la ciudad.\nEl presidente del gobierno ha asegurado que está tratando de solucionar la situación de todos\nlos jubilados que todavía no han recibido el dinero de las ayudas que prometió en marzo del\npasado año. El presidente culpa de todo lo sucedido a la Unión Europea, pues dice que nuestro\npaís sigue esperando al pago que se nos debe de todo el trimestre pasado.\nOtro presidente pidiendo disculpas, y es que en este caso Enrique Pérez, presidente de la\nempresa eléctrica IBERLUZ, ha emitido un comunicado oficial para asegurar que repasarán y\ncorregirán todas las facturas de los últimos cinco años. Muchos clientes ya han cancelado sus\ncontratos al sentirse engañados por la empresa.\nLos alumnos de la Universidad Internacional encontrarán un campus totalmente diferente el\npróximo curso. Según palabras del rector: “se ha hecho una inversión de tres millones de euros\npara poner césped por toda la universidad y renovar la zona de la cafetería”. Las obras\nempezarán esta misma semana y está previsto que duren alrededor de un mes.\nDesde hoy mismo los jubilados ya pueden hacer uso del nuevo centro social que se encuentra\nen la calle Libertad. Este lugar cuenta con mesas de billar, una zona para jugar a los dardos e\nincluso una pequeña bolera. El acceso es totalmente gratuito con la tarjeta de la tercera edad y,\npor el momento, el centro social solamente abrirá de jueves a domingo, de nueve de la mañana\na cinco de la tarde.\nConcluimos el informativo con un consejo para nuestros espectadores. Si descargáis la\naplicación de nuestra cadena en el teléfono móvil o tableta, tendréis acceso a toda la\ninformación de actualidad en directo. Además, en la aplicación se pueden ver los programas\nantiguos y los comentarios de otros usuarios.",
      "audio": "",
      "instrucciones": "Usted va a escuchar seis noticias en un programa. Escuche el programa dos veces. Después debe contestar a las preguntas (13-18). Seleccione la respuesta correcta (a / b / c). Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 19,
      "q_range": [
        19,
        24
      ],
      "answer": "F",
      "prompt": "Persona 19: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: F",
      "cuerpo": "",
      "transcript": "Estoy buscando un nuevo trabajo, escribí de nuevo mi currículum y ya he tenido\nalgunas entrevistas en empresas de logística. A todos los entrevistadores les digo lo mismo, que\nestoy disponible para incorporarme inmediatamente y que tengo vehículo propio. Tengo\nestudios y experiencia, creo que encontraré un empleo rápidamente.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 20,
      "q_range": [
        19,
        24
      ],
      "answer": "B",
      "prompt": "Persona 20: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "Ahora soy cocinero en un restaurante de comida asiática. Mi jefe me ha dicho\nque debo cortarme el pelo y afeitarme para poder continuar trabajando aquí. No me parece\njusto, pero lo voy a hacer porque no quiero quedarme en la calle, tengo que pagar el alquiler y\nnecesito mi sueldo.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 21,
      "q_range": [
        19,
        24
      ],
      "answer": "D",
      "prompt": "Persona 21: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: D",
      "cuerpo": "",
      "transcript": "Nuestro nuevo profesor de música es muy creyente, recuerdo un día que, en\nvez de darnos clase, nos llevó a todos a misa y estuvimos allí rezando durante cuarenta\nminutos. Otra vez nos llevó al Zoo para escuchar las melodías que hacen las aves. Sus clases\nson de todo menos aburridas.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 22,
      "q_range": [
        19,
        24
      ],
      "answer": "H",
      "prompt": "Persona 22: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: H",
      "cuerpo": "",
      "transcript": "Trabajé durante cinco años en el huerto de mi abuelo. Es un empleo muchísimo\nmás duro de lo que parece. Hay que levantarse todos los días a las cinco de la madrugada y,\nnormalmente, la jornada se acaba a las seis de la tarde. Además, en verano hay que regar dos\nveces al día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 23,
      "q_range": [
        19,
        24
      ],
      "answer": "E",
      "prompt": "Persona 23: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: E",
      "cuerpo": "",
      "transcript": "El otro día, al salir del trabajo, fui a tirar la basura como siempre. Tenía mi móvil\nen la mano y, cuando abrí el contenedor de reciclaje de vidrio, se me cayó el teléfono dentro,\nolía muy mal. Me metí dentro del contenedor para sacar el móvil, lo conseguí, pero me hice\ncortes en las dos manos.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 4,
      "q": 24,
      "q_range": [
        19,
        24
      ],
      "answer": "A",
      "prompt": "Persona 24: ¿Qué enunciado le corresponde?",
      "options": [
        {
          "key": "A",
          "text": "Habla de una guerra."
        },
        {
          "key": "B",
          "text": "Necesita ir a la peluquería."
        },
        {
          "key": "C",
          "text": "Viajó a un lugar muy tranquilo."
        },
        {
          "key": "D",
          "text": "Estuvo en la iglesia."
        },
        {
          "key": "E",
          "text": "Se hizo heridas con cristales."
        },
        {
          "key": "F",
          "text": "Puede empezar a trabajar ahora."
        },
        {
          "key": "G",
          "text": "Ha nacido alguien muy importante para esta persona."
        },
        {
          "key": "H",
          "text": "Dice que cultivaba plantas."
        },
        {
          "key": "I",
          "text": "Conoció a una persona en la clase de idiomas."
        }
      ],
      "type": "choice9",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "Un amigo mío es reportero, ahora vive y trabaja en Rabat porque hay un\nconflicto armado en la zona, es muy peligroso salir a la calle, hay soldados por todas partes y\nestá muriendo gente inocente. Mi amigo me dijo que el otro día explotó una bomba debajo de\nsu hotel.",
      "audio": "",
      "instrucciones": "Usted va a escuchar a seis personas que cuentan anécdotas. Escuche cada audio dos veces. Seleccione el enunciado (A-I) que corresponde a cada mensaje (del 19 al 24). Hay nueve opciones. Seleccione seis. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 25,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "Es presidente de una empresa.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 26,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "No tiene dinero en efectivo en este momento.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 27,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "Dice que algo va a ser más caro.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 28,
      "q_range": [
        25,
        30
      ],
      "answer": "A",
      "prompt": "Dice que venden productos a otro país.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: A",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 29,
      "q_range": [
        25,
        30
      ],
      "answer": "C",
      "prompt": "Va a ir a ver a la otra persona.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: C",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 2,
      "tarea": 5,
      "q": 30,
      "q_range": [
        25,
        30
      ],
      "answer": "B",
      "prompt": "Dice que la gente en Estados Unidos tiene más dinero cada año.",
      "options": [
        {
          "key": "A",
          "text": "NOÉ"
        },
        {
          "key": "B",
          "text": "MILA"
        },
        {
          "key": "C",
          "text": "NINGUNO"
        }
      ],
      "type": "choice3",
      "_v5": true,
      "explanation": "Respuesta correcta: B",
      "cuerpo": "",
      "transcript": "MUJER: Buenos días, mi nombre es Mila González, llamo del banco Cantabria. ¿Podría hablar\ncon el presidente de la empresa, por favor?\nHOMBRE: Disculpe, pero en este momento está en una reunión. Yo soy Noé, su secretario. ¿En\nqué puedo ayudarle?\nMUJER: Quería simplemente comentarle que su empresa debe dinero al banco e informarle de\nque hoy es el último día que puede pagar.\nHOMBRE: Pues hoy no creo que el presidente pueda ir en persona, ya que tiene todo el día\nocupado, pero se lo diré.\nMUJER: No es necesario que venga el señor presidente en persona, puede usted mismo venir al\nbanco con el dinero. En total se deben 278 €.\nHOMBRE: Pero yo aquí no tengo acceso a la caja de la empresa, ni tampoco tengo nada suelto\nen mi cartera.\nMUJER: Por favor, informe a su jefe de que esta situación provocará una subida de los precios\nde los productos que el banco ofrece a su empresa. Si no recuerdo mal, están interesados en\nlos créditos en dólares estadounidenses.\nHOMBRE: Sí, nuestra empresa exporta principalmente a Estados Unidos y estamos\ndesarrollando allí una campaña de publicidad en medios de comunicación.\nMUJER: De acuerdo, ¿puede decirle al señor presidente que me llame cuando pueda? Quiero\nenviarle por email un documento sobre la riqueza de los estadounidenses en los últimos tres\naños, que no para de crecer.\nHOMBRE: Creo que esto le va a interesar mucho. Se lo diré. Muchas gracias por su llamada.\nMUJER: A usted, que tenga un buen día.",
      "audio": "",
      "instrucciones": "Usted va a escuchar una conversación entre dos amigos. Indique si los enunciados (25-30) se refieren a Noé (A), a Mila (B) o a ninguno de los dos (C). Escuche la conversación dos veces. Marque las opciones elegidas en la Hoja de respuestas."
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 3,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        1
      ],
      "answer": "",
      "prompt": "Tarea 1: Usted recibe un email de una compañera de trabajo. Léalo y escriba",
      "options": [],
      "type": "writing",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Usted recibe un email de una compañera de trabajo. Léalo y escriba\nsu respuesta (entre 100 y 120 palabras).\nUsted debe:\nsaludar;\ncomentar lo que dice el cliente de Chile;\nconfirmar la asistencia a la reunión;\nopinar sobre la página web;\ndespedirse.\nBuenos días,\nMe han dicho que has hablado ya con el cliente de Chile. ¿Qué te ha dicho? Recuerda que nos\ninteresa mucho establecer una relación comercial con él, no hagas nada sin decírmelo antes a\nmí. Vamos a reunirnos a las 15:00 en mi despacho con la empresa de informática, quiero que\nestés tú también. Dime rápidamente, ¿qué opinas sobre el funcionamiento de nuestra web\nactual?\nRespóndeme cuanto antes,\nMarta, jefa del departamento comercial.",
      "rango_palabras": "100-120 palabras",
      "modelo_essay": "Hola Marta:\n\nMuchas gracias por tu mensaje, me alegro mucho de saber de ti. Siento que estes pasando por un momento dificil con el trabajo, pero creo que puedo darte un buen consejo.\n\nEn mi experiencia, cuando el ambiente laboral se vuelve toxico, lo mejor es buscar nuevas oportunidades sin prisas. Actualiza tu curriculum, crea un perfil en paginas de empleo y empieza a enviar solicitudes a empresas que realmente te interesen. Mientras tanto, intenta mantener una actitud positiva en tu puesto actual.\n\nRecuerdo que hace tres anos yo pase por algo parecido: mi jefe no valoraba mi trabajo y mis companeros competian constantemente. Decidi estudiar un curso online de marketing digital por las noches y, al cabo de seis meses, encontre un trabajo mucho mejor.\n\nEspero que mi consejo te sirva de ayuda. Si necesitas hablar, puedes llamarme cuando quieras.\n\nUn beso enorme,\nElena"
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 3,
      "tarea": 2,
      "q": 2,
      "q_range": [
        2,
        2
      ],
      "answer": "",
      "prompt": "Tarea 2: Elija solo una de las dos opciones que se le ofrecen a continuación y",
      "options": [],
      "type": "writing",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Elija solo una de las dos opciones que se le ofrecen a continuación y\nescriba un texto de entre 130 y 150 palabras:\nOPCIÓN 1\nLos deberes de esta semana son sencillos, debéis escribir un breve texto hablando sobre las\ntécnicas de estudio que utilizáis. Estos consejos pueden ser útiles para otros estudiantes, por favor,\nhacedlo lo mejor posible.\nTras leer un mensaje de su profesor, escriba un texto y:\ncomente cuáles son sus técnicas de estudio;\nhable del mejor lugar y momento para estudiar;\nponga un ejemplo de mala técnica de estudio.\nOPCIÓN 2\nTodos los trabajadores van a sufrir una reducción de sueldo del 7%, lo sentimos mucho, pero no\nnos queda otra opción, la empresa está sufriendo pérdidas económicas y estamos obligados a\nhacerlo. Empezaremos con el sueldo del próximo mes.\nUsted lee un comunicado de su empresa con el que no está de acuerdo, debe:\ndecir que no lo parece bien esta situación;\nexplicar los motivos;\nhablar sobre las consecuencias que esto puede tener.",
      "rango_palabras": "130-150 palabras",
      "modelo_essay": "Buenas noches a todos los estudiantes del foro:\n\nHoy quiero compartir mis tecnicas de estudio, que me han ayudado mucho durante la carrera. Lo primero que hago es organizarme con un calendario semanal: cada dia tiene asignadas dos o tres materias y nunca estudio mas de cuatro horas seguidas.\n\nMi lugar favorito para estudiar es la biblioteca de mi barrio. Prefiero ir por las mananas, de 9 a 13 horas, porque es cuando mi cerebro esta mas despierto y hay menos gente. Siempre llevo unos cascos con musica clasica de fondo, que me ayuda a concentrarme.\n\nUn ejemplo de mala tecnica de estudio que he sufrido fue estudiar la noche anterior al examen, sin haber dormido bien. Saque un 5 justo y me senti fatal durante una semana. Desde entonces, siempre duermo al menos ocho horas antes de cualquier prueba.\n\nEspero que estos consejos os sean utiles. !Mucho animo con los estudios!"
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 4,
      "tarea": 1,
      "q": 1,
      "q_range": [
        1,
        1
      ],
      "answer": "",
      "prompt": "Tarea 1: Le proponemos dos temas con algunas indicaciones para preparar",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Le proponemos dos temas con algunas indicaciones para preparar\nuna exposición oral. Elija uno de ellos. Tendrá que hablar durante 2 o\n3 minutos sobre el tema elegido. El entrevistador no intervendrá en\nesta parte de la prueba. Diferencie las partes de su exposición\n(introducción, desarrollo y conclusión final), ordene y relacione bien las\nideas y justifique sus opiniones y sentimientos.\nOPCIÓN 1\nHable sobre sus estudios, incluya información sobre:\ndónde estudió y por qué lo hizo en ese lugar;\ncon qué compañeros se llevaba mejor y por qué;\nsu opinión sobre alguno de sus profesores;\nalgo gracioso o curioso que le pasó durante sus estudios.\nOPCIÓN 2\nHaga un monólogo sobre el trabajo de sus sueños, mencione:\nqué tipo de trabajo es y dónde lo haría;\nqué tipo de compañeros tendría y cómo sería su relación;\ncuánto dinero ganaría;\ncómo sería un día normal en este trabajo.",
      "modelo_response": "Buenos dias, hoy voy a hablar sobre mi trabajo ideal y como me imagino mi futuro profesional.\n\nMi trabajo ideal seria trabajar como disenador grafico en una agencia de publicidad internacional. Desde pequeno, siempre me ha gustado dibujar y crear cosas nuevas, y descubri el diseno grafico cuando tenia quince anos.\n\nEn este trabajo, me gustaria tener la oportunidad de colaborar con profesionales de diferentes paises y culturas. Creo que la diversidad es fundamental para la creatividad y el aprendizaje continuo. Tambien valoraria mucho tener un horario flexible que me permita compaginar mi vida personal con la profesional.\n\nLas condiciones laborales ideales para mi incluyen un buen ambiente de trabajo, oportunidades de formacion y la posibilidad de crecer dentro de la empresa. Ademas, un salario justo y la opcion de trabajar desde casa algunos dias serian aspectos muy positivos.\n\nSi no pudiera trabajar en publicidad, mi segunda opcion seria ser profesor de arte en una escuela secundaria. Me encanta compartir conocimientos y ver como los jovenes desarrollan su creatividad.\n\nEn conclusion, creo que lo mas importante en un trabajo es sentirse realizado y feliz con lo que uno hace cada dia.",
      "rango_palabras": "2-3 min"
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 4,
      "tarea": 2,
      "q": 2,
      "q_range": [
        2,
        2
      ],
      "answer": "",
      "prompt": "Tarea 2: Cuando haya terminado su exposición (Tarea 1), usted deberá",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Cuando haya terminado su exposición (Tarea 1), usted deberá\nmantener una conversación con el entrevistador sobre el mismo tema\ndurante 3 o 4 minutos.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR:\nSI ELIGIÓ LA OPCIÓN 1\n¿Cuál era o es su asignatura preferida? ¿Y la que más problemas le ha dado durante su vida?\n¿Cómo iba todos los días a su escuela? ¿Se tenía que despertar muy pronto?\n¿Tenía o tiene algunas clases extra por las tardes? ¿De qué asignatura?\n¿Le gustaría trabajar en una escuela? ¿Por qué?\n¿Piensa que es mejor estudiar solo o en grupo? ¿Por qué?\nSI ELIGIÓ LA OPCIÓN 2\n¿Ha trabajado durante su vida? ¿En qué?\n¿Le gustaría ser jefe de una gran empresa? ¿Por qué?\n¿Podría trabajar en una fábrica? ¿O en la agricultura?\n¿Cree que hay diferencias entre los sueldos de hombres y mujeres en su país?\n¿Cuánto gana un camarero en su ciudad? ¿Crees que es un buen sueldo para vivir?",
      "modelo_response": "A continuacion voy a describir la fotografia de la reunion de trabajo.\n\nEn la imagen se puede ver una sala de reuniones moderna con una mesa grande de madera. Hay seis personas sentadas alrededor, tres hombres y tres mujeres, todos vestidos con ropa formal de oficina. Una mujer esta de pie senalando una pantalla donde se proyecta un grafico.\n\nEl lugar parece ser una oficina en un edificio alto, ya que a traves de los ventanales se ven otros edificios de la ciudad. La sala esta bien iluminada y decorada con plantas y cuadros abstractos.\n\nLas personas parecen estar discutiendo un proyecto importante. La mujer que habla tiene una expresion seria pero entusiasta, y los demas la escuchan con atencion. Sobre la mesa hay portatiles, libretas y tazas de cafe.\n\nImagino que este equipo esta preparando el lanzamiento de un nuevo producto o presentando los resultados trimestrales de la empresa. Por el lenguaje corporal, parece que la reunion va bien y todos estan comprometidos con el proyecto.\n\nEn mi experiencia laboral, he participado en muchas reuniones similares y se que la clave del exito es la preparacion previa y la comunicacion clara entre los miembros del equipo. Siempre llevo mis ideas organizadas y escucho atentamente las opiniones de mis companeros.",
      "rango_palabras": "3-4 min"
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 4,
      "tarea": 3,
      "q": 3,
      "q_range": [
        3,
        3
      ],
      "answer": "",
      "prompt": "Tarea 3: Le proponemos dos fotografías para esta tarea. Elija una, obsérvela y",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Le proponemos dos fotografías para esta tarea. Elija una, obsérvela y\ndescríbala con detalle, durante 1 o 2 minutos, lo que ve en la foto y lo\nque imagina que está ocurriendo.\nEstos son algunos aspectos que puede comentar:\nLas personas: dónde están, cómo son, qué hacen.\nEl lugar en el que se encuentran: cómo es.\nLos objetos: qué objetos hay, dónde están, cómo son.\nQué relación cree que existe entre las personas.\n¿Qué pasa, qué piensan o de qué hablan?\nPosteriormente, el entrevistador le hará algunas preguntas.\nLa duración total de esta tarea es de 2 a 3 minutos.\nOPCIÓN 1\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\n¿Le gustaría trabajar diseñando proyectos? ¿Por qué?\n¿Cuánto gana un arquitecto en su país? ¿Mucho o poco?\n¿Ha trabajado alguna vez en una oficina? ¿Qué le parece?\nOPCIÓN 2\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\n¿Conoce a alguien que trabaje a distancia?\n¿Cuáles son las ventajas y desventajas de trabajar a distancia? ¿Le gustaría tener este tipo de\ntrabajo?\n¿Cree que los trabajadores trabajan más o menos al hacerlo desde sus casas?\n¿Utiliza mucho el ordenador en su casa? ¿Qué suele hacer con él?",
      "modelo_response": "Vamos a analizar los datos sobre el consumo energetico en los hogares espanoles.\n\nEl grafico presenta la distribucion del consumo de energia en una vivienda tipica espanola. La calefaccion es, con diferencia, el mayor gasto energetico, representando un 47% del total. Le siguen los electrodomesticos con un 19%, el agua caliente con un 18% y la iluminacion con un 10%.\n\nLo que mas me llama la atencion es que la calefaccion suponga casi la mitad del consumo total. Esto significa que pequenos cambios en nuestros habitos de calefaccion pueden tener un gran impacto en el ahorro energetico y economico.\n\nEn mi opinion, es importante concienciar a la poblacion sobre la importancia de ahorrar energia. Pequenos gestos como bajar la calefaccion un grado, usar bombillas LED o comprar electrodomesticos eficientes pueden marcar una gran diferencia.\n\nAdemas, creo que el gobierno deberia ofrecer subvenciones para que las familias puedan mejorar el aislamiento de sus viviendas, ya que muchas casas en Espana tienen un aislamiento deficiente.\n\nEn conclusion, todos podemos contribuir a un consumo energetico mas responsable.",
      "rango_palabras": "2-3 min"
    },
    {
      "modelo": "nuevo_m4",
      "modelo_name": "Nuevo DELE B1 Modelo 4",
      "prueba": 4,
      "tarea": 4,
      "q": 4,
      "q_range": [
        4,
        4
      ],
      "answer": "",
      "prompt": "Tarea 4: Usted debe dialogar con el entrevistador en una situación simulada",
      "options": [],
      "type": "speaking",
      "_v5": true,
      "explanation": "",
      "cuerpo": "",
      "transcript": "",
      "audio": "",
      "instrucciones": "Usted debe dialogar con el entrevistador en una situación simulada\ndurante dos o tres minutos. La situación tiene relación con la imagen\nelegida en la tarea 3. La duración de esta tarea es de 2 a 3 minutos.\nSI ELIGIÓ LA OPCIÓN 1\nUsted está hablando con un cliente, la empresa en la que usted trabaja se va a retrasar con la\nentrega de unos productos, el cliente no puede esperar más y exige soluciones.\nDurante la conversación usted debe:\nsaludar educadamente a su cliente;\nexplicar los motivos por los que su empresa no podrá cumplir con lo prometido;\nofrecer algún tipo de solución;\npedir disculpas y ofrecer alguna compensación o garantía para el futuro.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\nSabe usted que necesitamos esos productos, ¿se puede saber qué ha pasado?\n¿Qué ofrecen ustedes para que continúe nuestra relación comercial?\n¿Cómo tendremos garantías de que esto no se repetirá en el futuro?\n¿Cuándo puede asegurarme que llegarán los productos?\nSI ELIGIÓ LA OPCIÓN 2\nUsted quiere irse de vacaciones y llevar el portátil de la empresa para poder trabajar desde el\nhotel. Hable con su jefe para pedirle permiso.\nDurante la conversación usted debe:\nexplicar la situación;\ndejar claro que va a trabajar igual que en su puesto de trabajo habitual;\nproponer otra alternativa para poder ir de vacaciones en estas fechas;\nhablar sobre algún caso anterior de otro trabajador.\nEJEMPLOS DE PREGUNTAS DEL ENTREVISTADOR\nBuenos días, ¿qué necesita?\n¿No cree usted que trabajará peor y la empresa saldrá perdiendo?\n¿No tenemos otra opción? No me convence mucho su propuesta.\n¿Alguna vez ha visto algo así en una empresa?",
      "modelo_response": "YO: Buenos días, señor Ruiz. Gracias por atenderme. Siento mucho lo que está pasando y le llamo precisamente para explicárselo.\nENTREVISTADOR: Sabe usted que necesitamos esos productos, ¿se puede saber qué ha pasado?\nYO: Ha habido un problema con nuestro proveedor de materiales: nos ha fallado un envío y la producción se ha retrasado una semana. No quiero poner excusas, solo que sepa exactamente qué ha ocurrido.\nENTREVISTADOR: ¿Qué ofrecen ustedes para que continúe nuestra relación comercial?\nYO: Podemos enviarle mañana, sin coste alguno, la mitad del pedido con nuestro transporte urgente, y el resto la semana que viene. Así no se queda sin existencias en ningún momento.\nENTREVISTADOR: ¿Cómo tendremos garantías de que esto no se repetirá en el futuro?\nYO: Hemos cambiado de proveedor y hemos añadido un segundo almacén de seguridad. Además, en el próximo pedido le aplicaremos un diez por ciento de descuento y firmaremos una cláusula de entrega con fecha garantizada.\nENTREVISTADOR: ¿Cuándo puede asegurarme que llegarán los productos?\nYO: La primera parte, mañana antes de las dos. La segunda, el miércoles de la semana que viene. Si quiere, le mando hoy mismo el número de seguimiento por correo.\nENTREVISTADOR: De acuerdo, confío en su palabra.\nYO: Muchas gracias por su paciencia. Le mantendré informado cada día hasta que reciba el pedido completo.",
      "rango_palabras": "2-3 min"
    }
  ]
};
