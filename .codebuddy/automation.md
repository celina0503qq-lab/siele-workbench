...
> **版本**: v5.1 (2026-09-13 · 新增「漏触发自愈与补推」机制 + 修正触发频率描述（业务周期仍为每 7 天出刊一期，实际触发改为每 2 天一次做自愈检查）+ 明确「日期 = 最近一个周五」不再等于触发当天 + 删除过期期号预告 + 补齐检查项 46/47)

> **版本优先级（重要）**：本文档含历史版本记录（v4.2/v4.3/v4.4/v4.5 章节）。**当历史记录中的数字或规则与「当前版本 v5.1」冲突时，一律以 v5.1 为准**。**本文档与 WorkBuddy 自动化任务的 prompt 冲突时，以任务 prompt 为准**（prompt 为运行时唯一权威来源，本文档为详细实现参考）。执行生成任务时，优先读「六之四（v4.4/v4.5）」、本节头部约束与下方调度安排，历史章节仅作背景参考。
...
> **实际触发频率：每 2 天一次**（`0 0 0 */2 * *`，用于漏期自愈检查）
...
1. 用 GitHub API 读取**线上** `refine_data.js`，取所有 `issue` 的最大值，记为 `MAX_ISSUE`；同时读出最新一期的日期 `LATEST_DATE`。
2. 计算应有期数：`SHOULD = MAX_ISSUE + floor((今天 − LATEST_DATE) / 7)`。
3. **若 `SHOULD == MAX_ISSUE`**：说明本期已完成或尚未到出刊日 → **直接结束，不推送、不发通知、不写任何文件**。
4. **若 `SHOULD > MAX_ISSUE`**：说明中间漏了 `(SHOULD − MAX_ISSUE)` 期 → **逐期补推**：
...
- **触发参数**：每 2 天一次，`0 0 0 */2 * *`（Asia/Shanghai），WorkBuddy 任务 `siele-v3`（id=5100839）
...
| 45 | **出刊周期每 7 天一期（日期取周五）；实际触发每 2 天一次（`0 0 0 */2 * *`）做自愈检查；勿用日字段 `*/N` 表达「每隔 N 天」** | 调度 |
| 46 | **执行前必做漏期自愈检查：读取线上 MAX_ISSUE/LATEST_DATE，`SHOULD==MAX_ISSUE` 则直接结束不推送；`SHOULD>MAX_ISSUE` 则逐期补推（日期取原定周五，期号连续不得跳号）** | 调度 |
...
*最后更新：2026-09-13 · v5.1：新增「漏触发自愈与补推」机制（见 §0.1），出刊周期仍为每 7 天（日期取周五），实际触发加密为每 2 天一次做自愈检查；明确「日期 = 最近一个周五，不是触发当天」；删除过期期号预告（原「下一期为 2026-09-04 第17期」已与事实脱节）；补齐检查项 46/47；明确本文档与任务 prompt 冲突时以 prompt 为准 · v5.0：出刊频率由每 4 天调整为每 7 天（每周五；吸取 `*/4` 语义歧义导致 8-28 漏触发的教训，弃用日字段 `*/N` 写法）· 其余规范基于 v4.5 不变*
