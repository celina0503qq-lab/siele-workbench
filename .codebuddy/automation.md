...
> **版本**: v4.5 (2026-08-20 · 阅读题题干/选项纯西语 + 解析定位原文 + B1/B2 难词 8–10 个 + B1/B2 每段词数 35–50（勿为扩写而扩写）+ B1/B2 禁取材 RTVE：El País/BBC Mundo/官方专页 + 内置页/独立页双入口来源一致性)

> **版本优先级（重要）**：本文档含历史版本记录（v4.2/v4.3/v4.4 章节）。**当历史记录中的数字或规则与「当前版本 v4.5」冲突时，一律以 v4.5 为准**。执行生成任务时，优先读「六之四（v4.4/v4.5）」和本节头部约束，历史章节仅作背景参考。
...
- **调度时间**：每 4 天 **00:00**（Asia/Shanghai，与自动化任务实际配置一致，v4.5 核对修正），由 WorkBuddy 自动化任务 `siele-v3`（id=5100839）触发
- **Cron 表达式**：`0 0 0 */4 * *`
...
*最后更新：2026-08-20 · v4.5：B1/B2 每段词数区间定为 35–50（勿为扩写而扩写）+ B1/B2 禁取材 RTVE（改用 El País/BBC Mundo/官方专页）+ 修正 6.6/6.6.1 来源规则 · 基于 v4.4 演进*
