...
> **版本**: v5.2 (2026-09-13 · 新增「幂等漏触发自愈与补推」机制（以产物存在性为裁判）+ 修正触发描述（业务周期仍为每 7 天出刊一期，实际触发为每天 08:00 一次）+ 明确「日期 = 最近一个周五」不再等于触发当天 + 删除过期期号预告 + 补齐检查项 46/47)

> **版本优先级（重要）**：本文档含历史版本记录（v4.2/v4.3/v4.4/v4.5 章节）。**当历史记录中的数字或规则与「当前版本 v5.2」冲突时，一律以 v5.1 为准**。**本文档与 WorkBuddy 自动化任务的 prompt 冲突时，以任务 prompt 为准**（prompt 为运行时唯一权威来源，本文档为详细实现参考）。执行生成任务时，优先读「六之四（v4.4/v4.5）」、本节头部约束与下方调度安排，历史章节仅作背景参考。

---

## 〇、调度安排与漏触发自愈（重要）

> **业务出刊频率：每 7 天一期，日期取周五**（亚洲/上海时区）
> **实际触发频率：每天 08:00 一次**（`0 0 8 * * *`，用于幂等漏期自愈检查；08:00 避开午夜边界抖动）
> **v5.1 变更（2026-09-13）**：新增「漏触发自愈与补推」机制；明确「日期 = 最近一个周五」；修正此前把触发频率与出刊频率混写的问题。
> **v4.3 变更（2026-08-16）**：B1 段落 ≥18 段、B2 段落 ≥20 段、B2 难题 ≥4/8、B1/B2 难词含详细分析字段。**自第 14 期（2026-08-20）起生效。**

### 0.1 漏触发自愈与补推（v5.1 新增 · 每次执行前必做）

由 WorkBuddy 自动化任务 `siele-v3`（id=5100839）触发。**因历史多次出现漏触发**（第16期迟 47 小时、第19期漏触发 69 小时），平台侧已将触发频率加密为**每天 08:00 一次**，并在任务 prompt 开头内置**幂等自愈检查块**。**执行任何生成动作之前，必须先完成以下自愈判定**：

1. 用 GitHub API 读取**线上** `refine_data.js`（**须绕开缓存**：加 `Cache-Control: no-cache` 或比对最新 commit 时间），取所有 `issue` 的最大值记为 `MAX_ISSUE`，并取最新一期日期 `LATEST_DATE`（必为周五）。
2. 计算**目标日期** `TARGET = LATEST_DATE + 7 天`（下一个应出的周五）。
3. **幂等判定（关键，取代旧的日期公式）**：检查 `articles/data/<TARGET>.js` 是否已存在、`REFINE_PACKS` 中是否已有 `date == TARGET` 的条目。**已存在 → 一律视为已推送，直接静默结束**。
   - 为何不用「`floor((今天−LATEST_DATE)/7)` 推期数」：该公式依赖「读取到的是最新数据」，若因缓存读到旧版本，会把已推的期误判为待补推，造成**重复推送**。以「产物是否存在」为裁判则天然幂等。
4. **仅当 `TARGET` 已过期未推（今天 ≥ TARGET 且产物不存在）时**才进入出刊流程：
   - 补推第 `K` 期（`K` 从 `MAX_ISSUE+1` 递增到 `SHOULD`）的日期 **= `LATEST_DATE` + 7 × (`K` − `MAX_ISSUE`) 天**（必落在周五，须核对），**不得挪到实际执行日**；
   - 期号必须从 `MAX_ISSUE+1` **连续递增、不得跳号**，全部补完为止；
   - **多期补推时各期选题必须互不重复**，并避开线上已有期次用过的来源文章；
   - 补推同样须走完整的生成 + 四重验证 + 4 文件推送流程，规范与正常期完全一致。
5. **期号一律实时读取线上计算**，禁止照抄本文档、`REFINE_PACKS` 本地副本或上一轮对话中的期号。
6. **前置异常处理**：若第 1 步的 GitHub API 读取失败（token 失效 / DNS 不通 / 404），必须重试至少 3 次或改用 WebFetch；**仍失败则立即终止并显式报告「自愈检查失败」，绝不允许静默结束**——静默结束会把真实漏期误判为无需出刊。

**为什么这么设计**：单次触发一旦被调度器错过，旧机制下会整周静默跳过且无任何报警。加密触发 + 幂等判定 + 补推规则三者组合后，任何一期漏触发都会在 ≤2 天内被自动发现并补齐，且不会重复推送。

### 0.2 调度参数

- **出刊周期**：每 7 天一期，**日期 = 最近一个周五**（不是触发当天）
- **触发参数**：每天 08:00 一次，`0 0 8 * * *`（Asia/Shanghai），WorkBuddy 任务 `siele-v3`（id=5100839）
- **平台调度能力限制（实测）**：WorkBuddy 的 `recurring` 类型会忽略 `INTERVAL`/`BYDAY`（归一为每天固定时刻），`interval` 类型只接受 `FREQ=HOURLY` 且 `INTERVAL∈[1,23]`。**因此「每 N 天」无法表达，只能用「每天固定时刻 + 幂等自愈」实现等效效果**
- **勿用日字段 `*/N` 表达「每隔 N 天」**：标准 cron 中日字段 `*/7` = 每月 1/8/15/22/29 日，`*/4` = 每月 1/5/9/13/…，**均非「每隔 N 天」**。历史上 v4.5 期因 `*/4` 语义歧义导致 8-28 漏触发。**v5.2 起不再尝试表达「每 N 天」**，改为每天 08:00 查岗 + 幂等判定（见 §0.1），从根本上绕开该语义陷阱。
- **期号连续性**：严格遵守「在线读取 `refine_data.js` 最大 `issue` + 1」，按实际推送次数递增
- **手动触发**：如需手动补推某一期，可临时创建 once 任务或手动执行生成流程（仍须遵守 v4/v5/v5.1 全部安全规则）
- **历史沿革**：v4.3（2026-08-13 起）由周一周四改为每 4 天按间隔触发 → v5.0（2026-08-29）调整为每 7 天（每周五）→ v5.1（2026-09-13）加自愈检查（每 2 天触发）→ v5.2（2026-09-13）自愈升级为**幂等版**，触发定为**每天 08:00**（平台不支持「每 N 天」）

---

## 一、模块概述

"外刊精炼"是西语 SIELE 备考工作台的定期推送模块（每 7 天更新一期），每期包含：
...
### 6.6 原文链接准确性
- `sourceUrl` 必须指向**内容与正文直接对应、可打开的页面**。**禁止**网站首页、频道页或列表页（如 RTVE 的 `rtve.es/noticias/economia/`——第 14 期 B1 曾因此「溯源链接没有明确指向」而被用户否决）
- **官方公告/计划专页可作为合法来源**（v4.5）：如 IDAE 的 `idae.es/ayudas-y-financiacion/programa-energias-renovables-innovadoras`——内容多、西语、正文事实与之直接对应，即使不是单篇文章页也允许（第 14 期 B1 正面案例）
- 生成时必须用 `curl -s -o /dev/null -w "%{http_code}" <url>` 验证返回 **200**，404/403/301 跳首页均不可用；沙箱 curl 连不通时改用 `WebFetch` 验证
...
### 6.6.1 B1/B2 选文来源硬性约束（v4.4/v4.5 · 第 14 期教训）
- **B1/B2 选文必须取自可打开的真实西语媒体/机构原文**，**严禁**使用日语/英语等他语种媒体（第 14 期 B2 曾误用 Japan Times，已被纠正）。
- **B1/B2 禁用 RTVE（v4.5）**：RTVE 的链接长期只有频道页/列表页、没有明确指向具体文章，**B1/B2 一律不取材 RTVE**。
- **来源优先级（B1/B2 严格执行）**：**El País 原文 > BBC Mundo 原文 > 官方机构/计划专页（如 IDAE）> 其他西语新闻媒体**。找不到上述可用原文时，宁可降低难度适配或换西语媒体，**也不得换用非西语媒体、不得用 RTVE**。
- **「尽量不改原文」的准确含义**（用户最终确认）：
...
- 来源优先选择：DELE Ahora（A1）→ RTVE（A2 仅限）→ BBC Mundo（B1/B2）→ El País（B2）。**B1/B2 禁取材 RTVE**（见 6.6.1）
- **每段词数区间（v4.5 · 用户明确指示，替代原 ≥40 硬指标）**：**B1/B2 每段西语 35–50 词都行**。**不要为了扩写而扩写**——35 词左右是正常水平，无需强行拉长到 40+；也不得为凑段数把长段拆成单句短段（第 14 期拆段教训）。校验：用 `段落 es.split(/\s+/).length` 断言每段在 35–50 之间；A1/A2 无此硬要求（入门级本就短句）。
...
- **在线日期计算（v5.1 必须）**：**当期日期 = 最近一个周五**（不是触发当天）。补推期的日期取**其原定的那个周五**，不得挪到实际执行日。计算前先确定今天是周几，再回推最近周五。
...
- **日期对应期号对照**：**不在此处硬编码**（历史版本曾写死「下一期为 2026-09-04 第17期」等预告，事后与事实脱节，v5.1 已删除）。**任何对照信息一律实时读取线上 `refine_data.js` 获取**，weekday 字段同步核对。
...
## 六之四、第 14 期实战修订（2026-08-20 · v4.4 → v4.5）

> 本节收录第 14 期（2026-08-20）生成后用户反馈的问题与修复，是 **v4.3 → v4.4** 的核心升级依据。**自第 15 期起必须严格执行。**
> **v4.5（同日二次修订）**：① B1/B2 每段词数区间定为 **35–50 词**（替代 v4.4 的「≥40」，**不要为了扩写而扩写**）；② **B1/B2 禁取材 RTVE**（溯源链接无明确指向），改为 El País > BBC Mundo > 官方机构/计划专页（如 IDAE）；③ 6.6 补充「官方公告/计划专页可作为合法来源」；④ **SKILL 同步规则**：仓库 `skills/siele-daily-refine-pack/SKILL.md` 是外刊精炼的 SKILL 文档，本地系统 `~/.codebuddy/skills/` 是实际加载路径——**更新仓库版 SKILL 后必须运行 `bash devext/sync_skill.sh` 同步到本地**（本地不会自动跟随仓库更新；该脚本已入库 `devext/sync_skill.sh`）。
...
| 43 | **B1/B2 每段西语 35–50 词（勿为凑数扩写，也勿拆段凑数）** | 数据 |
| 44 | **B1/B2 禁取材 RTVE（溯源链接无明确指向）；优先 El País > BBC Mundo > 官方专页** | 数据 |
| 45 | **出刊周期每 7 天一期（日期取周五）；实际触发每天 08:00 一次（`0 0 8 * * *`）做幂等自愈检查；平台不支持「每 N 天」，故用每天查岗+幂等判定实现等效** | 调度 |
| 46 | **执行前必做幂等自愈：算 `TARGET = LATEST_DATE + 7天`，若 `articles/data/<TARGET>.js` 已存在则静默结束；不存在且已过期则出刊（日期取 TARGET，期号连续不得跳号），推完重新读线上循环直到 TARGET > 今天** | 调度 |
| 47 | **期号与日期一律实时读取线上计算，禁止照抄本文档/`REFINE_PACKS` 本地副本/上一轮对话；本文档与任务 prompt 冲突时以 prompt 为准** | 调度 |
| 48 | **自愈检查失败（GitHub API 读不到）时禁止静默结束，必须重试后显式报告失败；A1/A2 难词 5 个仅 {w,m}，B1/B2 难词 8–10 含 ejemplo/analisis；补推日期 = LATEST_DATE + 7×(K−MAX_ISSUE)** | 调度/数据 |

---

*最后更新：2026-09-13 · v5.2：自愈逻辑升级为**幂等版**（以 `articles/data/<TARGET>.js` 是否存在为裁判，取代易受缓存影响的日期公式，杜绝重复推送），实际触发定为每天 08:00（平台不支持「每 N 天」）；明确「日期 = 最近一个周五，不是触发当天」；删除过期期号预告（原「下一期为 2026-09-04 第17期」已与事实脱节）；补齐检查项 46/47；明确本文档与任务 prompt 冲突时以 prompt 为准 · v5.0：出刊频率由每 4 天调整为每 7 天（每周五；吸取 `*/4` 语义歧义导致 8-28 漏触发的教训，弃用日字段 `*/N` 写法）· 其余规范基于 v4.5 不变*
