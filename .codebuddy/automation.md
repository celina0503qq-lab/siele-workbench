...
由 WorkBuddy 自动化任务 `siele-v3`（id=5100839）触发。**因历史多次出现漏触发**（第16期迟 47 小时、第19期漏触发 69 小时），平台侧已将触发频率加密为每 2 天一次，并在任务 prompt 开头内置自愈检查块。**执行任何生成动作之前，必须先完成以下自愈判定**：
...
- **勿用日字段 `*/N` 表达「每隔 N 天」**：标准 cron 中日字段 `*/7` = 每月 1/8/15/22/29 日，`*/4` = 每月 1/5/9/13/…，**均非「每隔 N 天」**。历史上 v4.5 期因 `*/4` 语义歧义导致 8-28 漏触发。当前配置的 `*/2` 仅用于「每 2 天查岗」，其语义恰好等价于隔天，可安全使用。
- **期号连续性**：严格遵守「在线读取 `refine_data.js` 最大 `issue` + 1」，按实际推送次数递增
- **手动触发**：如需手动补推某一期，可临时创建 once 任务或手动执行生成流程（仍须遵守 v4/v5/v5.1 全部安全规则）
- **历史沿革**：v4.3（2026-08-13 起）由周一周四改为每 4 天按间隔触发 → v5.0（2026-08-29）调整为每 7 天（每周五）→ v5.1（2026-09-13）出刊周期保持每 7 天（周五），另加每 2 天触发做自愈检查
