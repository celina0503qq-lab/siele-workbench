...
> **版本优先级（重要）**：本文档含历史版本记录（v4.2/v4.3/v4.4/v4.5 章节）。**当历史记录中的数字或规则与「当前版本 v5.1」冲突时，一律以 v5.1 为准**。**本文档与 WorkBuddy 自动化任务的 prompt 冲突时，以任务 prompt 为准**（prompt 为运行时唯一权威来源）。执行生成任务时，优先读「六之四（v4.4/v4.5）」、本节头部约束与下方调度安排，历史章节仅作背景参考。
...
4. **若 `SHOULD > MAX_ISSUE`**：说明中间漏了 `(SHOULD − MAX_ISSUE)` 期 → **逐期补推**：
   - 补推期的**日期取其原定的周五**（不得挪到实际执行日），期号从 `MAX_ISSUE + 1` 连续递增，全部补完为止；
