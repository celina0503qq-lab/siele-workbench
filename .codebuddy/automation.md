...
- **每段词数硬指标（v4.4 · 第 14 期教训）**：**B1/B2 每段西语 ≥40 词**（avg 目标 40–55），不得为凑段数把长段拆成单句短段。历史参考：11–13 期 B1 avg 38–49、B2 avg 47–55。校验：用 `段落 es.split(/\s+/).length` 断言每段 ≥40；A1/A2 无此硬要求（入门级本就短句）。
...
| 43 | **B1/B2 每段西语 ≥40 词（不得拆段凑数，avg 40–55）** | 数据 |

---

*最后更新：2026-08-20 · v4.4：阅读题题干/选项纯西语 + 解析定位原文（标段落号）+ B1/B2 难词提升至 8–10 个 + B1/B2 选文来源硬约束（El País/BBC Mundo 真实原文）+ 内置页/独立页双入口来源一致性校验 + 认证改用 hosts.yml oauth_token（弃 fine-grained PAT/gh）+ 难题标签化可机器校验 + B1/B2 每段 ≥40 词硬指标 · 基于 v4.3 演进*
