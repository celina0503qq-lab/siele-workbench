...
> **版本**: v4.1 (2026-08-12 · 更新频率调整为每周一、周四)

---

## 〇、调度安排（重要）

> **更新频率：每周一、周四各更新一期**（自 2026-08-12 起，替代原先的每日更新）

- **调度时间**：周一、周四 **09:00**（Asia/Shanghai），由 WorkBuddy 自动化任务 `siele-v3`（id=5100839）触发
- **Cron 表达式**：`0 0 9 * * 1,4`（1=周一，4=周四）
- **每期对应日期**：周一推送覆盖周一、周二、周三三天的外刊；周四推送覆盖周四、周五、周六、周日四天的外刊
- **期号连续性**：依然严格遵守"在线读取 refine_data.js 最大 issue + 1"的规则，期号按实际推送次数递增（不分周一/周四），不按自然日
- **手动触发**：如需手动补推某一期，可临时创建 once 任务或手动执行生成流程（仍须遵守 v4 全部安全规则）
- **不再每日触发**：任何每日触发的调度配置已移除，避免与周一/周四频率冲突

---

## 一、模块概述

"外刊精炼"是西语 SIELE 备考工作台的定期推送模块（每周一、周四更新），每期包含：
...
*最后更新：2026-08-12 · 更新频率调整为每周一、周四（v4.1）· 基于 11 期实际运行 + Quiz 持久化与云同步 + 期号连续性/HTML 内联安全经验总结*
