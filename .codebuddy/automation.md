...
> **版本**: v5.0 (2026-08-29 · 更新频率由每 4 天调整为每 7 天（每周五）+ cron 改为 `0 0 0 * * 5`；其余沿用 v4.5 全部规范：阅读题题干/选项纯西语 + 解析定位原文 + B1/B2 难词 8–10 个 + B1/B2 每段词数 35–50（勿为扩写而扩写）+ B1/B2 禁取材 RTVE：El País/BBC Mundo/官方专页 + 内置页/独立页双入口来源一致性)

> **版本优先级（重要）**：本文档含历史版本记录（v4.2/v4.3/v4.4/v4.5 章节）。**当历史记录中的数字或规则与「当前版本 v5.0」冲突时，一律以 v5.0 为准**。执行生成任务时，优先读「六之四（v4.4/v4.5）」、本节头部约束与下方调度安排，历史章节仅作背景参考。

---

## 〇、调度安排（重要）

> **更新频率：每 7 天更新一期**（每周五，v5.0 起由每 4 天调整为每 7 天）
> **v4.3 变更（2026-08-16）**：B1 段落 ≥18 段、B2 段落 ≥20 段、B2 难题 ≥4/8、B1/B2 难词含详细分析字段。**自第 14 期（2026-08-20）起生效。**

- **调度时间**：每 7 天（每周五）**00:00**（Asia/Shanghai），由 WorkBuddy 自动化任务 `siele-v3`（id=5100839）触发
- **Cron 表达式**：`0 0 0 * * 5`（每周五 00:00。**勿用 `*/7`**——标准 cron 中日字段 `*/7`=每月 1/8/15/22/29 日，不是「每隔 7 天」，正是 v4.5 期 `*/4` 语义歧义导致 8-28 漏触发的教训）
- **每期对应日期**：触发当天即为当期日期（周五），期号按实际推送次数递增
- **期号连续性**：依然严格遵守"在线读取 refine_data.js 最大 issue + 1"的规则，期号按实际推送次数递增
- **手动触发**：如需手动补推某一期，可临时创建 once 任务或手动执行生成流程（仍须遵守 v4/v5 全部安全规则）
- **频率变更说明（v5.0，2026-08-29）**：应用户要求由「每 4 天」调整为「每 7 天（每周五）」；上一期 8-28（第16期，周五）为每 4 天制最后一期，**下一期为 2026-09-04（第17期，周五）**，之后每周五触发。历史沿革：v4.3（2026-08-13 起）由周一周四改为每 4 天按间隔触发
...
| 45 | **更新频率每 7 天一期（每周五 00:00，cron `0 0 0 * * 5`，勿用 `*/7`）** | 调度 |

---

*最后更新：2026-08-29 · v5.0：更新频率由每 4 天调整为每 7 天（每周五 00:00，cron `0 0 0 * * 5`；吸取 `*/4` 语义歧义导致 8-28 漏触发的教训，弃用日字段 `*/N` 写法）· 其余规范基于 v4.5 不变*
